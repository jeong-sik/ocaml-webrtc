...
let start_handshake t ~remote_host ~remote_port =
  ...
  | Error e ->
    t.state <- Error e;
    Printf.eprintf "[DTLS-SCTP] Handshake start failed: %s\n%!" e
...
let rec process_dtls_record t data =
  ...
  | Ok (response_records, app_data_opt) ->
    ...
    if Dtls.is_established t.dtls && t.state = Handshaking then begin
      t.state <- Connected;
  ...
  | Error e ->
    Printf.eprintf "[DTLS-SCTP] Record processing error: %s\n%!" e;
    t.state <- Error e
...
and process_sctp_outputs t outputs =
  List.iter (fun output ->
    ...
    | Sctp_core.SendPacket packet ->
      ...
      | Error e ->
        Printf.eprintf "[DTLS-SCTP] Encrypt error: %s\n%!" e
      end
...
let open_channel t ~label ?protocol ?priority ?channel_type ()
  ...
  if t.state <> Connected then
    Result.error "DTLS not connected"
  else begin
