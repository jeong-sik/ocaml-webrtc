...
let next_sequence seq = (seq + 1) land 0xFFFF

let encode header ~payload =
  if header.version <> 2
  then Error "RTP version must be 2"
  else if header.payload_type < 0 || header.payload_type > 127
  then Error "RTP payload_type must be 0..127"
  else if header.sequence < 0 || header.sequence > 0xFFFF
  then Error "RTP sequence must be 0..65535"
  else if List.length header.csrc > 15
  then Error "RTP CSRC count must be <= 15"
  else (
    let ext_len =
      ...
      | Some ext ->
        ...
        if data_len mod 4 <> 0 then -1 else 4 + data_len
    in
    if ext_len = -1
    then Error "RTP extension data length must be multiple of 4"
    else if header.padding_len < 0
    then Error "RTP padding_len must be >= 0"
    else (
      ...
      if header.padding_len > 0 && total_len < header_len + 1
      then Error "RTP padding_len invalid"
      else (
        let buf = Bytes.create total_len in
        let b0 =
          ((header.version land 0x3) lsl 6)
          lor (if header.padding || header.padding_len > 0 then 0x20 else 0)
          lor (if header.extension <> None then 0x10 else 0)
          lor (List.length header.csrc land 0x0F)
        in
        let b1 =
          (if header.marker then 0x80 else 0) lor (header.payload_type land 0x7F)
        in
...
let decode data =
  ...
  if len < 12
  then Error "RTP packet too short"
  else (
    ...
    if version <> 2
    then Error "RTP version mismatch"
    else if len < 12 + (4 * csrc_count)
    then Error "RTP packet missing CSRC list"
    else (
      ...
      let extension =
        ...
        then Error "RTP packet missing extension header"
        else (
          ...
          if len < ext_start + ext_len
          then Error "RTP packet missing extension data"
          else (
      ...
      | Ok ext ->
        ...
        then Error "RTP payload offset invalid"
        else (
          let padding_len =
            if not padding
            then 0
          ...
          in
          if padding && (padding_len = 0 || padding_len > len - payload_start)
          then Error "RTP padding length invalid"
          else (
            let payload_end = len - padding_len in
            let payload_len = payload_end - payload_start in
            if payload_len < 0
            then Error "RTP payload length invalid"
            else (
  ...
  then Error "RTP packet too short"
    ...
    then Error "RTP packet missing CSRC list"
      ...
      | Ok ext ->
        ...
        let padding_len =
          if not padding then 0 else if len = 0 then 0 else Bytes.get_uint8 data (len - 1)
        in
        let header =
          { version
          ; padding
