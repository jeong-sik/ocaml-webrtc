...
let parse_media_line line =
  ...
  | media :: port_str :: proto :: formats ->
    (match
       media_type_of_string media, protocol_of_string proto, int_of_string_opt port_str
     with
     | Some mt, Some p, Some port ->
       Ok
         { media_type = mt
         ; port
         ; num_ports = None
...
let parse_candidate line =
  ...
  | foundation
    ...
    :: rest ->
    (match
       int_of_string_opt comp, Int64.of_string_opt priority, int_of_string_opt port_str
     with
...
let parse_ice_options = function
  | Some v -> split_on_char ' ' v |> List.filter (fun s -> s <> "")
  | None -> []
;;
...
let parse sdp =
  ...
  let rec parse_lines session current_media = function
    ...
    | line :: rest when String.length line >= 2 ->
      ...
      let value =
        if String.length line > 2 then String.sub line 2 (String.length line - 2) else ""
      in
      (match prefix, current_media with
       (* Session-level *)
       | "v=", None ->
         parse_lines
           { session with version = int_of_string_opt value |> Option.value ~default:0 }
           None
           rest
       | "o=", None ->
       ...
       | "s=", None -> parse_lines { session with session_name = value } None rest
       | "c=", None ->
                ...
                (match attr_value with
                 | Some v ->
                   { m with
                     sctpmap =
                       Some
                         { port = int_of_string_opt v |> Option.value ~default:5000
              ...
              | "group" ->
                (match attr_value with
                 | Some v ->
                   ...
                   (match parts with
                    | semantics :: ids ->
                      { session with groups = session.groups @ [ semantics, ids ] }
                    | _ -> session)
                 | None -> session)
              | _ ->
                { session with
                  other_attrs = session.other_attrs @ [ attr_name, attr_value ]
                }
            in
...
let rec to_string session =
  ...
  Option.iter
    ...
    session.connection;
  List.iter
    (fun t -> add (Printf.sprintf "t=%Ld %Ld" t.start_time t.stop_time))
    session.timings;
  ...
  then add (Printf.sprintf "a=ice-options:%s" (String.concat " " session.ice_options));
  Option.iter
    (fun fp -> add (Printf.sprintf "a=fingerprint:%s %s" fp.hash_func fp.fingerprint))
    session.fingerprint;
  List.iter
    (fun (sem, ids) -> add (Printf.sprintf "a=group:%s %s" sem (String.concat " " ids)))
    session.groups;
  ...
  List.iter
    (fun m ->
       ...
       if m.ice_options <> []
       then add (Printf.sprintf "a=ice-options:%s" (String.concat " " m.ice_options));
       Option.iter
         (fun fp ->
            add (Printf.sprintf "a=fingerprint:%s %s" fp.hash_func fp.fingerprint))
         m.fingerprint;
       Option.iter (fun v -> add (Printf.sprintf "a=setup:%s" v)) m.setup;
       Option.iter (fun v -> add (Printf.sprintf "a=mid:%s" v)) m.mid;
       Option.iter
         (fun (sm : sctpmap) -> add (Printf.sprintf "a=sctp-port:%d" sm.port))
         m.sctpmap;
       Option.iter
         (fun v -> add (Printf.sprintf "a=max-message-size:%d" v))
         m.max_message_size;
       Option.iter (fun v -> add (Printf.sprintf "a=%s" v)) m.direction;
       List.iter
         (fun c -> add (Printf.sprintf "a=%s" (candidate_to_string c)))
         m.ice_candidates;
...
and candidate_to_string c =
  let base =
    Printf.sprintf
      "candidate:%s %d %s %Ld %s %d typ %s"
      c.foundation
      c.component_id
      (String.lowercase_ascii c.transport)
      c.priority
      c.address
      c.port
      c.cand_type
  in
  let with_rel =
    match c.rel_addr, c.rel_port with
...
let create_datachannel_offer ~ice_ufrag ~ice_pwd ~fingerprint ~sctp_port =
  ...
  ; origin =
      { username = "-"
      ; sess_id = Int32.to_string (Int32.abs (Webrtc_crypto.random_int32 ()))
      ; sess_version = 1L
      ; net_type = IN
      ; addr_type = IP4
  ...
  ; media =
      [ { media_type = Application
        ...
        ; sctpmap =
            Some
              { port = sctp_port; protocol = "webrtc-datachannel"; streams = Some 1024 }
  ...
  ; media =
      List.map
        (fun m ->
           { m with
             setup = Some "active"
           ; (* Answer is active if offer is actpass *)
...
let restart_ice ~session ~ice_ufrag ~ice_pwd ?(ice_options = []) () =
  ...
  let update_media (m : media) =
    ...
    { m with ice_ufrag = Some ice_ufrag; ice_pwd = Some ice_pwd; ice_options = merged }
  in
...
let add_candidate session candidate ~media_index =
  if media_index >= 0 && media_index < List.length session.media
  then (
    let media =
      List.mapi
        (fun i m ->
           if i = media_index
           then { m with ice_candidates = m.ice_candidates @ [ candidate ] }
           else m)
        session.media
    in
    { session with media })
...
let get_candidates session = List.concat_map (fun m -> m.ice_candidates) session.media
...
let has_datachannel session =
  List.exists
    (fun m ->
       m.media_type = Application && (m.protocol = DTLS_SCTP || m.protocol = UDP_DTLS_SCTP))
    session.media
;;

let pp_session fmt session = Format.fprintf fmt "%s" (to_string session)
let pp_media fmt media = Format.fprintf fmt "%s" (media_to_string media)
...
module OfferAnswer = struct
  ...
  let create () = { state = Stable; local_description = None; remote_description = None }
  let state t = t.state
  ...
  let create_offer t media_list =
    ...
    else if not (can_create_offer t)
    then
      Error
  ...
  let create_answer t media_list =
    if t.state = Closed
    then Error AlreadyClosed
    else if not (can_create_answer t)
    then
      Error
        (InvalidStateTransition
  ...
  let set_local_description t desc =
    ...
    then Error AlreadyClosed
      ...
      (* Rollback *)
      | HaveLocalOffer, Rollback -> Ok { t with state = Stable; local_description = None }
      (* Invalid transitions *)
      ...
      | _, _ ->
        Error
          (InvalidStateTransition
             (Printf.sprintf
                "Invalid local %s in state %s"
                (match desc.sdp_type with
                 | Offer -> "offer"
                 | Answer -> "answer"
                 | Pranswer -> "pranswer"
                 | Rollback -> "rollback")
  ...
  let set_remote_description t desc =
    ...
    then Error AlreadyClosed
      ...
      | HaveLocalOffer, Offer -> Error GlareDetected
      (* Invalid transitions *)
      | Stable, Answer ->
