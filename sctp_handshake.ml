...
let compute_hmac data =
  ...
  then
    Log.warn
      "[SECURITY WARNING] Using default HMAC secret - set SCTP_HMAC_SECRET for production";
...
let verify_hmac data expected_hmac = Bytes.equal (compute_hmac data) expected_hmac
...
let encode_cookie cookie =
  ...
  for i = 0 to 7 do
    Bytes.set
      buf
      i
      (Char.chr (Int64.to_int (Int64.shift_right_logical time_bits (i * 8)) land 0xFF))
  done;
  (* Lifespan *)
...
let decode_cookie buf =
  if Bytes.length buf <> 68
  then Error "Invalid cookie length"
  else (
    (* Extract fields *)
    ...
    (* Verify HMAC *)
    let data_portion = Bytes.sub buf 0 36 in
    if not (verify_hmac data_portion hmac)
...
let encode_init_ack params cookie =
  ...
  Bytes.set_int16_be result 20 7;
  (* State Cookie parameter type *)
  Bytes.set_int16_be result 22 72;
  (* 4 + 68 *)
...
let decode_init buf =
  ...
  then Error "INIT chunk too short"
  else (
    let chunk_type = Char.code (Bytes.get buf 0) in
    if chunk_type <> chunk_type_init && chunk_type <> chunk_type_init_ack
    then Error (Printf.sprintf "Not an INIT chunk (type=%d)" chunk_type)
    else
      Ok
...
let decode_cookie_echo buf =
  if Bytes.length buf < 72
  then Error "COOKIE-ECHO too short"
  else (
    let chunk_type = Char.code (Bytes.get buf 0) in
    if chunk_type <> chunk_type_cookie_echo
    then Error "Not a COOKIE-ECHO chunk"
    else decode_cookie (Bytes.sub buf 4 68))
;;
...
let client_process_init_ack init_ack_buf local_params =
  if Bytes.length init_ack_buf < 92
  then Error "INIT-ACK too short"
  else (
       ...
       | Ok cookie ->
         ...
         let assoc =
           ...
           ; num_outbound_streams =
               min local_params.num_outbound_streams peer_params.num_inbound_streams
           ; num_inbound_streams =
               min local_params.num_inbound_streams peer_params.num_outbound_streams
           }
         in
...
let client_process_cookie_ack cookie_ack_buf assoc =
  if Bytes.length cookie_ack_buf < 4
  then Error "COOKIE-ACK too short"
  else (
    let chunk_type = Char.code (Bytes.get cookie_ack_buf 0) in
    if chunk_type <> chunk_type_cookie_ack
    then Error "Not a COOKIE-ACK chunk"
    else (
...
let pp_association fmt assoc =
  Format.fprintf
    fmt
    "Association{state=%s, local_vtag=%ld, peer_vtag=%ld, local_tsn=%ld, peer_tsn=%ld}"
    (state_to_string assoc.state)
    assoc.local_vtag
    assoc.peer_vtag
    assoc.local_initial_tsn
    assoc.peer_initial_tsn
;;
