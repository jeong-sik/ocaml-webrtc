...
let classify_packet data =
  ...
  then Unknown_packet
    ...
    then Stun_packet
    else if first_byte >= 20 && first_byte <= 23
    then Dtls_packet
    else Unknown_packet)
;;
...
let set_remote_credentials t ~ufrag ~pwd = Ice.set_remote_credentials t.ice ~ufrag ~pwd

(** Get local ICE credentials *)
let get_local_credentials t = Ice.get_local_credentials t.ice

(** Get local candidates *)
let get_local_candidates t = Ice.get_local_candidates t.ice

(** Signal end of remote candidates *)
let set_remote_end_of_candidates t = Ice.set_remote_end_of_candidates t.ice
  ...
  | Some pair ->
    (match t.local_socket with
     | Some sock ->
       let dest_addr =
         Unix.ADDR_INET
           (Unix.inet_addr_of_string pair.Ice.remote.address, pair.remote.port)
       in
       let _ = Unix.sendto sock data 0 (Bytes.length data) [] dest_addr in
       ()
     | None -> Log.warn "[ICE-DTLS] No local socket bound")
  | None -> Log.warn "[ICE-DTLS] No nominated pair yet"
;;

(** Send DTLS records via ICE *)
let send_records t records = List.iter (send_via_ice t) records
  ...
  | Ok msg ->
    (match msg.Stun.msg_class, msg.Stun.msg_method with
     | Stun.Request, Stun.Binding ->
       ...
       let exists =
         List.exists
           (fun c -> c.Ice.address = from_ip && c.port = from_port)
           t.ice.Ice.remote_candidates
       in
       if not exists
       then (
         let prflx =
           Ice.create_prflx_candidate ~component:1 ~address:from_ip ~port:from_port ()
         in
         Ice.add_remote_candidate t.ice prflx);
       let mapped_address : Stun.address =
         { family = Stun.IPv4; port = from_port; ip = from_ip }
       in
       let response =
...
let rec process_dtls_record t data =
  ...
  | Ok (response_records, app_data_opt) ->
    ...
    if Dtls.is_established t.dtls && t.state = Dtls_handshaking
    then (
      t.state <- Established;
    ...
    (match app_data_opt with
     ...
     | None -> ())
  | Error e ->
    Log.error "[ICE-DTLS] DTLS error: %s" e;
    t.state <- Failed e
    ...
    (fun output ->
       ...
       | Sctp_core.SendPacket packet ->
         ...
         (match Dtls.encrypt t.dtls packet with
          ...
          | Error e -> Log.error "[ICE-DTLS] Encrypt error: %s" e)
...
let start_dtls_handshake t =
  ...
  (match Ice.get_nominated_pair t.ice with
   | Some pair ->
     ...
     let local_addr =
       Unix.ADDR_INET (Unix.inet_addr_of_string pair.Ice.local.address, pair.local.port)
     in
     Unix.bind sock local_addr;
     t.local_socket <- Some sock;
     t.local_addr <- Some (pair.local.address, pair.local.port)
   | None -> Log.warn "[ICE-DTLS] No nominated pair for DTLS");
  ...
  then (
    ...
    | Error e ->
      t.state <- Failed e;
      Log.error "[ICE-DTLS] DTLS handshake start failed: %s" e)
;;
...
let handle_incoming t ~data ~from_addr =
  ...
  | Stun_packet -> process_stun_packet t data ~from_addr
  | Dtls_packet -> process_dtls_record t data
  | Unknown_packet ->
    Log.warn
      "[ICE-DTLS] Unknown packet type (first byte: 0x%02x)"
