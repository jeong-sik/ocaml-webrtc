...
let create ?(config = default_config) () =
  ...
  let dtls_config =
    if config.is_controlling
    then Dtls.default_client_config
...
let add_remote_candidate t candidate = Ice.add_remote_candidate t.ice candidate
...
let send_through_ice t data =
  ...
  | None -> Error "No nominated ICE pair"
;;
...
let send_dtls_records t records =
  List.iter
    (fun record ->
       ...
       | Ok _ -> ()
       | Error e -> Log.error "[ICE-DTLS] Send failed: %s" e)
    records
;;
...
let rec process_dtls_record t data =
  ...
  let result =
    if t.config.is_controlling
    then Dtls.handle_record t.dtls data
    else (
      ...
      | None -> Dtls.handle_record t.dtls data (* Fallback *))
  in
  match result with
  | Ok (response_records, app_data_opt) ->
    ...
    if response_records <> [] then Dtls.store_flight t.dtls response_records;
    (* Check if handshake completed *)
    if Dtls.is_established t.dtls && t.state = HandshakingDTLS
    then (
    ...
    (match app_data_opt with
     ...
     | None -> ())
  | Error e ->
    Log.error "[ICE-DTLS] Record processing error: %s" e;
    set_error t e
...
and send_sctp_packet t packet =
  ...
  | Ok encrypted -> ignore (send_through_ice t encrypted)
  | Error e -> Log.error "[ICE-DTLS-SCTP] Encrypt error: %s" e
...
and process_sctp_packet t packet =
  ...
  then (
    ...
    | 1 when not t.config.is_controlling ->
      ...
      (match Sctp_handshake.server_process_init packet with
       | Ok (_local_params, init_ack) -> send_sctp_packet t init_ack
       | Error e ->
         Log.error "[SCTP] INIT processing error: %s" e;
         set_error t ("SCTP INIT: " ^ e))
    | 2 when t.config.is_controlling ->
      (* INIT-ACK received - we're client *)
      (match t.sctp_init_params with
       | Some local_params ->
         (match Sctp_handshake.client_process_init_ack packet local_params with
          | Ok (assoc, cookie_echo) ->
            t.sctp_association <- Some assoc;
            send_sctp_packet t cookie_echo
          | Error e ->
            Log.error "[SCTP] INIT-ACK processing error: %s" e;
            set_error t ("SCTP INIT-ACK: " ^ e))
       | None -> Log.warn "[SCTP] Received INIT-ACK but no init_params stored")
    | 10 when not t.config.is_controlling ->
      (* COOKIE-ECHO received - we're server *)
      (match Sctp_handshake.server_process_cookie_echo packet with
       | Ok (assoc, cookie_ack) ->
         t.sctp_association <- Some assoc;
       ...
       | Error e ->
         Log.error "[SCTP] COOKIE-ECHO processing error: %s" e;
         set_error t ("SCTP COOKIE-ECHO: " ^ e))
    | 11 when t.config.is_controlling ->
      (* COOKIE-ACK received - we're client, handshake complete! *)
      (match t.sctp_association with
       | Some assoc ->
         (match Sctp_handshake.client_process_cookie_ack packet assoc with
          | Ok updated_assoc ->
            t.sctp_association <- Some updated_assoc;
            (* Client-side: connection established after receiving COOKIE-ACK *)
            set_state t Connected
          | Error e ->
            Log.error "[SCTP] COOKIE-ACK processing error: %s" e;
            set_error t ("SCTP COOKIE-ACK: " ^ e))
       | None -> Log.warn "[SCTP] Received COOKIE-ACK but no association stored")
    | _ ->
...
and process_sctp_outputs t outputs =
  List.iter
    (fun output ->
       ...
       | Sctp_core.Error e -> set_error t ("SCTP: " ^ e))
    outputs
...
let connect t : (unit, string) result Lwt.t =
  ...
  Lwt.bind (Ice.run_connectivity_checks t.ice) (fun () ->
    ...
    if ice_state = Ice.Failed
    then (
      set_error t "ICE failed: all checks failed";
      Lwt.return (Result.Error "ICE failed"))
    else if
      (not has_nominated) && ice_state <> Ice.Connected && ice_state <> Ice.Completed
    then (
...
let open_channel t ~label ?protocol ?priority ?channel_type () : (int, string) result =
  ...
  then Result.error "Not connected"
  else (
...
let handle_dtls_timeout t =
  ...
  | Ok flight when flight <> [] -> send_dtls_records t flight
  | Ok _ -> ()
  | Error e -> set_error t ("DTLS timeout: " ^ e)
;;

(** Check if DTLS retransmit is needed (for polling) *)
let check_dtls_retransmit t = Dtls.check_retransmit_needed t.dtls
...
let handle_incoming t data =
  ...
  | _ -> Log.warn "[ICE-DTLS] Unexpected packet in state %s" (string_of_state t.state)
;;
...
let close t =
  ...
  then (
    ...
    | Some alert -> ignore (send_through_ice t alert)
    | None -> ());
  set_state t Closed
;;
