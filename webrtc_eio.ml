...
let create ?(ice_config = default_ice_config) ~role () =
  ...
  ; dtls = (if is_client then Dtls_eio.create_client () else Dtls_eio.create_server ())
  ...
  List.iter
    (function
      ...
      | Sctp_core.DeliverData { stream_id; data } ->
        ...
        (match List.find_opt (fun ch -> ch.id = stream_id) t.channels with
...
let create_datachannel t ~label =
  ...
  t.next_channel_id <- t.next_channel_id + 2;
  (* Even/odd increment *)
  let channel = { id; label; on_message = None; on_open = None; on_close = None } in
  t.channels <- channel :: t.channels;
  (* Send DCEP OPEN message *)
  let dcep_open =
    Dcep.encode_open
      { Dcep.channel_type = Dcep.Reliable; priority = 0; label; protocol = "" }
  in
  let outputs =
...
let add_ice_candidate t candidate = Ice_eio.add_remote_candidate t.ice candidate
