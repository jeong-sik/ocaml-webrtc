...
let create () = {
  ...
  rtt_smoothed = 0.2;  (* Initial guess: 200ms *)
...
let on_packet_acked t ~tsn =
  Hashtbl.remove t.xmit_records tsn
...
let update_rtt t ~rtt_sample =
  (* Update min RTT *)
  if rtt_sample < t.rtt_min then
    t.rtt_min <- rtt_sample;

  (* Update smoothed RTT (Jacobson/Karels) *)
  if t.rtt_smoothed = 0.2 then
    t.rtt_smoothed <- rtt_sample
  else
    t.rtt_smoothed <- 0.875 *. t.rtt_smoothed +. 0.125 *. rtt_sample;

  (* Update reorder window: min(rtt/4, 200ms), at least 1ms *)
  t.reorder_window <-
    Float.max min_reorder_window
...
let detect_loss t ~now ~acked_tsns =
  ...
  List.iter (fun tsn ->
    ...
    | Some info ->
      ...
      if rtt_sample > 0.0 && info.retx_count = 0 then
        update_rtt t ~rtt_sample;

      (* Update most recent ACK tracking *)
      if Int32.compare tsn t.most_recent_ack_tsn > 0 then begin
  ...
  Hashtbl.iter (fun tsn info ->
    ...
    if Int32.compare tsn t.most_recent_ack_tsn < 0 then begin
      ...
      if time_since_sent > loss_threshold then
        lost_tsns := tsn :: !lost_tsns
    end
  ) t.xmit_records;
...
let pp fmt t =
  Format.fprintf fmt "RACK{rtt_min=%.3fs, srtt=%.3fs, reorder=%.3fs, in_flight=%d}"
    t.rtt_min t.rtt_smoothed t.reorder_window (Hashtbl.length t.xmit_records)

(** {1 Backward-Compatible API for sctp_reliable.ml} *)

(** Record packet send (alias for on_packet_sent) *)
let record_send t tsn =
  on_packet_sent t ~tsn ~size:1024 ~now:(Unix.gettimeofday ())
...
let process_sack t ~cumulative_tsn ~gap_blocks =
  ...
  Hashtbl.iter (fun tsn info ->
    if Int32.compare tsn cumulative_tsn <= 0 then begin
      acked_tsns := tsn :: !acked_tsns;
      (* Calculate RTT sample from first ACKed packet *)
      if !rtt_sample = None && info.retx_count = 0 then begin
  ...
  List.iter (fun (gap_start, gap_end) ->
    ...
    let rec add_range tsn =
      if Int32.compare tsn end_tsn <= 0 then begin
        acked_tsns := tsn :: !acked_tsns;
