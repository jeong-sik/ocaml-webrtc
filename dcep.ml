...
let create ~is_client = {
  ...
  next_stream_id = if is_client then 0 else 1;  (* Even for client, odd for server *)
  is_client;
}
...
let decode_open buf =
  if Bytes.length buf < 12 then
    Error "DATA_CHANNEL_OPEN too short"
  else begin
    let msg_type = Bytes.get buf 0 |> Char.code in
    if msg_type <> msg_type_data_channel_open then
      Error (Printf.sprintf "Expected OPEN (0x03), got 0x%02x" msg_type)
    else begin
      ...
      if Bytes.length buf < 12 + label_len + proto_len then
        Error "DATA_CHANNEL_OPEN truncated"
      else begin
        let label = Bytes.sub_string buf 12 label_len in
        let protocol = Bytes.sub_string buf (12 + label_len) proto_len in

        let channel_type = match channel_type_code with
          | 0x00 -> Reliable
          ...
          | code ->
            ...
            Printf.eprintf "[DCEP] Unknown channel type 0x%02x, defaulting to Reliable\n%!" code;
            Reliable
        in
...
let allocate_stream_id t =
  ...
  t.next_stream_id <- t.next_stream_id + 2;  (* Skip by 2 to maintain even/odd *)
  id

(** Create and open a new DataChannel *)
let open_channel t ~label ?(protocol = "") ?(priority = 256)
...
let handle_open t ~stream_id (open_msg : data_channel_open) =
  let channel = {
    id = stream_id;
    state = Open;  (* Immediately open upon receiving OPEN *)
...
let handle_ack t ~stream_id =
  ...
  | Some channel ->
    ...
    Ok ()
  | None ->
    Error (Printf.sprintf "ACK for unknown channel %d" stream_id)
...
(** Get channel by ID *)
let get_channel t ~stream_id =
  Hashtbl.find_opt t.channels stream_id
...
let list_channels t =
  Hashtbl.fold (fun _ ch acc ->
    if ch.state = Open then ch :: acc else acc
  ) t.channels []
...
let ppid_for_data ~is_string channel =
  let base_ppid =
    if is_string then 51l  (* WebRTC String *)
    else 53l  (* WebRTC Binary *)
  in
...
let get_stats t =
  let active = Hashtbl.fold (fun _ ch count ->
    if ch.state = Open then count + 1 else count
  ) t.channels 0 in
  {
...
let string_of_channel_type = function
  ...
  | PartialReliableRexmitUnordered n -> Printf.sprintf "PartialReliableRexmitUnordered(%d)" n
  | PartialReliableTimed n -> Printf.sprintf "PartialReliableTimed(%dms)" n
  | PartialReliableTimedUnordered n -> Printf.sprintf "PartialReliableTimedUnordered(%dms)" n
...
let pp_channel ch =
  Printf.sprintf "Channel[%d]: %s (%s) state=%s priority=%d"
    ch.id ch.label (string_of_channel_type ch.channel_type)
    (string_of_state ch.state) ch.priority
