...
let gather_candidates_full agent =
  ...
  let stun_servers =
    List.filter
      (fun url -> String.length url >= 5 && String.sub url 0 5 = "stun:")
      stun_urls
  in
         ...
         List.map
           (fun stun_server ->
              ...
              | Some srflx ->
                ...
                let exists =
                  List.exists
                    (fun c -> c.address = srflx.address && c.port = srflx.port)
                    agent.local_candidates
                in
                if not exists
                then (
  ...
  let turn_urls =
    List.concat_map
      (fun server ->
         List.map
           (fun url -> url, server.username, server.credential, server.tls_ca)
           server.urls)
  ...
  let relay_promises =
    List.concat_map
      (fun host ->
         List.map
           (fun (turn_server, username, credential, tls_ca) ->
              ...
              | Some relay ->
                ...
                let exists =
                  List.exists
                    (fun c -> c.address = relay.address && c.port = relay.port)
                    agent.local_candidates
                in
                if not exists
                then (
...
let execute_check_output agent output =
  ...
  | Ice_check.Set_timer { duration_ms } ->
    ...
    Lwt.return_unit
  | Ice_check.Cancel_timer -> Lwt.return_unit
  | Ice_check.Check_completed { success = _; nominated = _; mapped_addr = _; error = _ }
    -> Lwt.return_unit
  | Ice_check.No_op -> Lwt.return_unit
;;
...
let check_pair agent pair =
  ...
  let check_config =
    { Ice_check.default_config with max_attempts = agent.config.max_check_attempts }
  in
  let check =
    Ice_check.create
...
let run_connectivity_checks agent =
  ...
  let pairs =
    List.map (fun pair -> { pair with pair_state = Pair_waiting }) agent.pairs
  in
  agent.pairs <- pairs;
      ...
      (fun pair ->
         ...
         | Check_success succeeded_pair ->
           ...
           <- List.map
                (fun p ->
                   if
                     p.local.address = pair.local.address
                     && p.local.port = pair.local.port
                     && p.remote.address = pair.remote.address
                     && p.remote.port = pair.remote.port
                   then succeeded_pair
                   else p)
                agent.pairs;
           (* Check for nomination *)
           if succeeded_pair.nominated then agent.nominated_pair <- Some succeeded_pair;
           Lwt.return_unit
         | Check_failure _ ->
...
let nominate_pair agent pair =
  let open Lwt.Infix in
  if agent.config.role <> Controlling
  then Lwt.return (Error "Only controlling agent can nominate")
  else if pair.pair_state <> Pair_succeeded
  then Lwt.return (Error "Can only nominate succeeded pairs")
  else (
    let remote_ufrag = Option.value ~default:"" agent.remote_ufrag in
    let remote_pwd = Option.value ~default:"" agent.remote_pwd in
    ...
    >>= fun () ->
    (* Wait for response *)
    Lwt_unix.sleep (float_of_int agent.config.ta_timeout_ms /. 1000.0)
    >>= fun () ->
    let now = Unix.gettimeofday () in
    let response =
      Ice_check.Stun_response_received
    ...
    | Ice_check.Failed ->
      let error =
        Option.value ~default:"Nomination failed" (Ice_check.get_error final_check)
      in
      Lwt.return (Error error)
    | _ -> Lwt.return (Error "Nomination did not complete"))
;;
...
let refresh_consent agent =
  ...
  | Some pair when agent.consent_expired -> Lwt.return (Error "Consent already expired")
  | Some pair ->
    let remote_ufrag = Option.value ~default:"" agent.remote_ufrag in
    let remote_pwd = Option.value ~default:"" agent.remote_pwd in
    ...
    Lwt_unix.sleep 0.5
    >>= fun () ->
    if Ice_check.get_state check = Ice_check.Succeeded
    then (
