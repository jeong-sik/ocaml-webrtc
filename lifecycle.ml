...
let advance t ~phase ~cleanup =
  match t.state with
  | Failed _ ->
    Error "cannot advance: lifecycle is in failed state"
  | Active current ->
    let cur_idx = phase_index current in
    let new_idx = phase_index phase in
    if new_idx <> cur_idx + 1 then
      Error (Printf.sprintf "invalid transition: %s → %s (must be sequential)"
               (show_phase current) (show_phase phase))
    else begin
...
let fail t ~error =
  ...
  let cleaned_phases = List.map (fun entry ->
    (try entry.cleanup ()
     with _exn -> ());  (* cleanup must not propagate exceptions *)
    entry.phase
  ) t.completed in
...
let completed_phases t =
  List.rev_map (fun e -> e.phase) t.completed
