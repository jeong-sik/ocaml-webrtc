...
type output =
  ...
  | Set_timer of { duration_ms : int (** Timer duration in milliseconds *) }
...
let default_config = { initial_rto_ms = 500; max_rto_ms = 3000; max_attempts = 7 }
...
let create
  ...
  in
  let context = { local_addr; remote_addr; stun_attrs; transaction } in
  { state = Frozen; context; nominated = false; last_error = None }
;;

(** {1 State Transitions (Pure Functions)} *)

(** Calculate next RTO with exponential backoff *)
let next_rto ~max_rto_ms current_rto = min (current_rto * 2) max_rto_ms
...
let step (check : t) (input : input) (now : float) : t * output =
  ...
  | Waiting, Start_check ->
    let transaction = { check.context.transaction with attempt = 1; sent_at = now } in
  ...
  | In_progress, Stun_response_received { transaction_id; success = true; mapped_addr; _ }
    ->
    if Bytes.equal transaction_id check.context.transaction.id
    then (
      let check =
        { check with
          state = Succeeded
      ...
      let output =
        Check_completed
          { success = true; nominated = check.nominated; mapped_addr; error = None }
      in
      check, output)
    else
  ...
  | In_progress, Stun_response_received { transaction_id; success = false; error_code; _ }
    ...
    then (
      let error_msg =
        ...
        | None -> "Unknown error"
      in
      let check = { check with state = Failed; last_error = Some error_msg } in
      let output =
        Check_completed
          { success = false
          ; nominated = false
  ...
  | In_progress, Timer_fired ->
    ...
    then (
      ...
      let check =
        { check with state = Failed; last_error = Some "Timeout after max retries" }
      in
      let output =
        Check_completed
          { success = false
      ...
      let transaction =
        { tx with attempt = tx.attempt + 1; rto_ms = new_rto; sent_at = now }
      in
      let context = { check.context with transaction } in
  ...
  (* Any state: Cancel *)
  | _, Cancel ->
    let check = { check with state = Failed; last_error = Some "Cancelled" } in
    check, Cancel_timer
  (* Terminal states or invalid transitions *)
  | Succeeded, _ | Failed, _ -> check, No_op
  | _, _ -> check, No_op
;;
...
let create_check_list () = { checks = []; next_id = 0 }
...
(** Get check by ID *)
let get_check list id = List.assoc_opt id list.checks

(** Update check in list *)
let update_check list id check =
  let checks = List.map (fun (i, c) -> if i = id then i, check else i, c) list.checks in
  { list with checks }
;;

(** Get next waiting check (for scheduling) *)
let get_next_waiting list =
  List.find_opt (fun (_, check) -> check.state = Waiting) list.checks
;;

(** Get all succeeded checks *)
let get_succeeded list =
  List.filter_map
    (fun (id, check) -> if check.state = Succeeded then Some (id, check) else None)
    list.checks
;;
...
let all_finished list = List.for_all (fun (_, check) -> is_terminal check) list.checks
