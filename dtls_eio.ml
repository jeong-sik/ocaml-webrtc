...
let make_io_ops ~clock =
  { Dtls.send = (fun _ -> 0)
  ; (* We handle sends manually via records *)
...
let send_records t records =
  ...
  | Some send -> List.iter (fun record -> send record) records
  | None -> ()
;;
...
let handle_record t ~clock record =
  ...
  Dtls.run_with_io ~ops:io_ops (fun () ->
    ...
    | Ok (records, app_data) ->
      ...
      Option.iter (fun data -> Option.iter (fun f -> f data) t.on_data) app_data;
      (* Check if handshake just completed *)
      if Dtls.is_established t.dtls
      then (
...
let send t data =
  if not (Dtls.is_established t.dtls)
  then Error "Handshake not complete"
  else (
...
let decrypt t data = Dtls.decrypt t.dtls data
...
let run t ~sw:_ ~clock ~role ~client_addr ~on_established =
  ...
  | `Client ->
    (match run_client_handshake t ~clock ~timeout_s:30.0 with
     ...
     | Error e -> Option.iter (fun f -> f e) t.on_error)
  | `Server ->
    (match run_server_handshake t ~clock ~client_addr ~timeout_s:30.0 with
     | Ok () -> on_established ()
     | Error e -> Option.iter (fun f -> f e) t.on_error)
;;
