...
type recv_buffer =
  ...
  ; mutable dup_tsns : int32 list
    (** Duplicate TSNs received since last SACK (RFC 4960 §6.2) *)
  }
...
let create ?(config = Sctp.default_config) ?initial_tsn () =
  ...
  ; (* Receiver's cumulative_tsn must be (initial_tsn - 1) so that
    ...
    recv_buf =
      { cumulative_tsn = Int32.pred initial_tsn
      ; (* 999 -> expects 1000 *)
        gap_ranges = []
      ; (* O(k) gap tracking - no Hashtbl! *)
...
(** Decode SACK chunk *)
let decode_sack buf =
  if Bytes.length buf < sack_chunk_header_size
  then Error "SACK too short"
  else (
    let chunk_type = Bytes.get buf 0 |> Char.code in
    if chunk_type <> 3
    then Error (Printf.sprintf "Not a SACK chunk: type=%d" chunk_type)
    else (
...
let record_received t tsn =
  ...
  let record_dup () =
    if List.length recv.dup_tsns < 4 && not (List.mem tsn recv.dup_tsns)
    then recv.dup_tsns <- tsn :: recv.dup_tsns
  in
  (* Only process TSNs > cumulative - if not, it's a duplicate *)
  if not (tsn_gt tsn recv.cumulative_tsn)
  then (
    record_dup ();
    ...
    then (
      ...
      let rec insert_tsn = function
        ...
        | ({ gr_start; gr_end } as range) :: rest ->
          if tsn_lt tsn gr_start
          then
            (* TSN comes before this range *)
            if Int32.succ tsn = gr_start
            then
          ...
          else if tsn_le tsn gr_end
          then
            (* TSN already in this range (duplicate) *)
            range :: rest, false
          else if tsn = Int32.succ gr_end
          then (
...
let generate_sack t =
  ...
  let dup_tsns = List.rev recv.dup_tsns in
  (* Oldest first *)
  recv.dup_tsns <- [];
  { cumulative_tsn_ack = recv.cumulative_tsn; a_rwnd = recv.a_rwnd; gap_blocks; dup_tsns }
;;
...
let process_sack t sack =
  ...
  List.iter
    (fun gap ->
       for offset = gap.start_offset to gap.end_offset do
         ...
         if not (Sctp_ring_buffer.is_acked t.rtx_queue tsn)
         then (
  ...
  let gap_block_list = List.map (fun g -> g.start_offset, g.end_offset) sack.gap_blocks in
  ...
  List.iter
    (fun lost_tsn ->
       if Sctp_ring_buffer.mark_retransmit t.rtx_queue lost_tsn
       then t.stats.fast_retransmissions <- t.stats.fast_retransmissions + 1)
    rack_lost_tsns;
  ...
  if sack.gap_blocks <> []
  then
    Sctp_ring_buffer.iter_unacked_above t.rtx_queue cum_tsn (fun _tsn miss_count ->
  ...
  then
    ...
    then
      ...
      if cc.partial_bytes_acked >= cc.cwnd
      then (
        cc.partial_bytes_acked <- cc.partial_bytes_acked - cc.cwnd;
        cc.cwnd <- cc.cwnd + mtu));
  (* Cancel T3-rtx timer if all data acked *)
  if Sctp_ring_buffer.is_empty t.rtx_queue then t.t3_rtx_start <- None
;;
...
let can_send t = Sctp_ring_buffer.flight_size t.rtx_queue < t.cc.cwnd
...
let alloc_tsn t = Sctp_ring_buffer.alloc_tsn t.rtx_queue
...
let pp_cc_state fmt t =
  Format.fprintf
    fmt
    "cwnd=%d ssthresh=%d flight=%d rto=%.3fs"
    t.cc.cwnd
    t.cc.ssthresh
    (Sctp_ring_buffer.flight_size t.rtx_queue)
    t.rto.rto
;;
...
let get_gap_ranges t =
  List.map
    (fun { gr_start; gr_end } -> Int32.to_int gr_start, Int32.to_int gr_end)
