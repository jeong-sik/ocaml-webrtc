...
let encode_stream_list streams =
  ...
  List.iteri (fun i sid ->
    write_uint16_be buf (i * 2) sid
  ) streams;
  buf
...
let decode_stream_list buf =
  ...
  if len mod 2 <> 0 then
    Error "Invalid stream list length"
  else
...
let encode_param = function
  ...
  | Unknown (param_type, body) ->
    build_param param_type body

let encode_params params =
  Bytes.concat Bytes.empty (List.map encode_param params)
...
let decode_param param_type body =
  ...
  | t when t = param_outgoing_ssn_reset || t = param_incoming_ssn_reset ->
    if body_len < 12 then Error "Reset request too short"
    else
      ...
      (match decode_stream_list stream_bytes with
       | Error e -> Error e
       | Ok streams ->
         if t = param_outgoing_ssn_reset then
           Ok (Outgoing_ssn_reset { request_seq; response_seq; last_tsn; streams })
         else
           Ok (Incoming_ssn_reset { request_seq; response_seq; last_tsn; streams }))
  | t when t = param_reconfig_response ->
    if body_len < 8 then Error "Reconfig response too short"
    else
  ...
  | t when t = param_add_outgoing || t = param_add_incoming ->
    if body_len < 8 then Error "Add streams parameter too short"
    else
      let request_seq = read_uint32_be body 0 in
      let new_streams = read_uint16_be body 4 in
      if t = param_add_outgoing then
        Ok (Add_outgoing_streams { request_seq; new_streams })
      else
        Ok (Add_incoming_streams { request_seq; new_streams })
  | _ ->
    Ok (Unknown (param_type, body))

let decode_params buf =
  let len = Bytes.length buf in
  let rec loop offset acc =
    if offset + 4 > len then Ok (List.rev acc)
    else
      let param_type = read_uint16_be buf offset in
      let param_len = read_uint16_be buf (offset + 2) in
      if param_len < 4 || offset + param_len > len then
...
let of_raw_chunk (chunk : Sctp.raw_chunk) =
  if chunk.chunk_type <> Sctp.int_of_chunk_type Sctp.RE_CONFIG then
    Error "Not a RE-CONFIG chunk"
  else
    decode_params chunk.chunk_value
