...
let create ?(config = default_config) () =
  let buffers = Array.init config.pool_size (fun _ -> Bytes.create config.buffer_size) in
...
*)
let[@inline] get_bytes buf = buf.data, buf.pool.config.buffer_size
...
*)
let[@inline] set_len buf len = buf.len <- len
...
*)
let[@inline] get_len buf = buf.len
...
*)
let blit_to buf ~dst ~dst_off = Bytes.blit buf.data 0 dst dst_off buf.len
