...
let estimate_roc t seq =
  ...
  | None -> t.recv_roc
  | Some last ->
    if seq < last && last - seq > 0x8000 then Int32.succ t.recv_roc else t.recv_roc
;;
...
let protect_rtp t ?(marker = false) ~timestamp ~payload () =
  ...
  | Ok packet ->
    (match
       Srtp.protect_rtp ~profile:t.profile ~keys:t.local_keys ~roc:t.send_roc ~packet
     with
     | Ok _ as ok ->
       advance_send_seq t;
       ok
...
let unprotect_rtp t ~packet =
  if Bytes.length packet < 4
  then Error "RTP packet too short"
  else (
    let seq = read_uint16_be packet 2 in
    let roc = estimate_roc t seq in
    match Srtp.unprotect_rtp ~profile:t.profile ~keys:t.remote_keys ~roc ~packet with
    | Error e -> Error e
    | Ok plaintext ->
