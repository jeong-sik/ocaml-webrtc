...
let send_connected t ~data =
  if t.closed
  then Error "Transport closed"
  else if t.remote_endpoint = None
...
let recv_timeout t ~buf ~timeout_ms =
  ...
  then Error "Transport closed"
    ...
    if ready = [] then Error "Timeout" else recv t ~buf)
;;
...
let pp_endpoint fmt ep = Format.fprintf fmt "%s:%d" ep.host ep.port

let pp_stats fmt s =
  Format.fprintf
    fmt
    "sent=%d/%dB recv=%d/%dB errors=%d/%d"
    s.packets_sent
    s.bytes_sent
    s.packets_recv
    s.bytes_recv
