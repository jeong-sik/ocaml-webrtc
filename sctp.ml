...
type config = {
  ...
  skip_checksum_validation: bool;  (** TESTING ONLY: Skip CRC32c validation.
...
let default_config = {
  ...
  rto_initial_ms = 1000;
  rto_min_ms = 200;      (* More aggressive min RTO *)
  rto_max_ms = 60000;
  (* Larger receive window for high bandwidth-delay product *)
  a_rwnd = 262144;       (* 256KB - enables better pipelining *)
  ...
  let rec bundle acc current_buf current_len = function
    | [] ->
      if current_len > 0 then
        List.rev (Bytes.sub current_buf 0 current_len :: acc)
      else
        List.rev acc
    | dc :: rest ->
      let encoded = encode_data_chunk dc in
      let encoded_len = Bytes.length encoded in
      if current_len + encoded_len <= mtu then begin
      ...
      end else if current_len > 0 then begin
        (* Start new packet *)
...
let fragment_data ~data ~stream_id ~stream_seq ~ppid ~start_tsn ~mtu =
  let max_data_per_chunk = mtu - 16 in (* 16 = DATA chunk header *)
  let data_len = Bytes.length data in
  if data_len <= max_data_per_chunk then
    ...
    let rec make_fragments acc idx tsn offset =
      if idx >= num_fragments then
        ...
        make_fragments (fragment :: acc) (idx + 1) (Int32.succ tsn) (offset + chunk_data_len)
    in
...
let decode_data_chunk buf =
  decode_data_chunk_view buf ~off:0 ~len:(Bytes.length buf)
