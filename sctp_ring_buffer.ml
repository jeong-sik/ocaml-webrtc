...
let enqueue t chunk =
  if is_full t then
    ...
    t.count <- t.count + 1;
    t.flight_bytes <- t.flight_bytes + data_size + 16;  (* +16 for header *)
    Some tsn
  end
...
let get t tsn =
  ...
  match t.entries.(idx) with
  | InFlight entry when entry.chunk.tsn = tsn ->
    Some (entry.chunk, entry.retransmit_count)
  | _ -> None
...
let process_cumulative_ack t cum_tsn now =
  ...
  while Int32.compare !tsn cum_tsn <= 0 &&
        Int32.compare !tsn t.tail_tsn < 0 do
    let idx = tsn_to_index t !tsn in
    (match t.entries.(idx) with
    | InFlight entry when entry.chunk.tsn = !tsn ->
      let data_size = Bytes.length entry.chunk.Sctp.user_data in
      ...
      if entry.retransmit_count = 0 && !rtt_sample = None then
        rtt_sample := Some (now -. entry.sent_at);
...
let iter_unacked_above t base_tsn f =
  ...
  for _ = 0 to t.count - 1 do
    ...
    | InFlight entry when Int32.compare !tsn base_tsn > 0 ->
      entry.miss_indications <- entry.miss_indications + 1;
      if f !tsn entry.miss_indications then
        entry.fast_retransmit <- true
    | _ -> ());
    tsn := Int32.succ !tsn
  done
...
let stats t =
  Printf.sprintf "RingBuffer{head=%ld, tail=%ld, count=%d/%d, flight=%d bytes}"
    t.head_tsn t.tail_tsn t.count t.capacity t.flight_bytes
