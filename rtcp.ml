...
let read_int24_be buf off =
  ...
  if b0 land 0x80 <> 0 then v - 0x1000000 else v
;;
...
let encode_sdes_chunk (chunk : sdes_chunk) =
  ...
  List.iter
    (fun item ->
       ...
       Bytes.set_uint8 buf !pos item_type;
       incr pos;
...
let decode_sdes_chunk buf off limit =
  if off + 4 > limit
  then Error "RTCP SDES chunk truncated"
  else (
    let ssrc = read_uint32_be buf off in
    ...
    while (not !saw_end) && !pos < limit && !error = None do
      ...
      then error := Some "RTCP SDES item header truncated"
      else (
        let len = Bytes.get_uint8 buf (!pos + 1) in
        if !pos + 2 + len > limit
        then error := Some "RTCP SDES item truncated"
        else (
    ...
    | None ->
      if not !saw_end
      then Error "RTCP SDES missing end marker"
      else (
...
let encode packet =
  let packet_type, count, body_len, write_body =
    ...
    | Source_description chunks ->
      ...
      let body_len =
        List.fold_left (fun acc b -> acc + Bytes.length b) 0 encoded_chunks
      in
      let write_body buf off =
        let pos = ref off in
        List.iter
    ...
    | App app ->
      if app.subtype < 0 || app.subtype > 31
      then invalid_arg "RTCP APP subtype must be 0..31";
      if String.length app.name <> 4
      then invalid_arg "RTCP APP name must be exactly 4 characters";
      let data_len = Bytes.length app.data in
      if data_len mod 4 <> 0
      then invalid_arg "RTCP APP data length must be 32-bit aligned";
      let body_len = 4 + 4 + data_len in
      (* SSRC + name + data *)
      let write_body buf off =
    ...
    | Unknown_packet (pt, data) ->
      ...
      let write_body buf off = Bytes.blit data 0 buf off body_len in
      pt, 0, body_len, write_body
  in
  if body_len mod 4 <> 0 then invalid_arg "RTCP body length must be 32-bit aligned";
...
let decode data =
  ...
  then Error "RTCP packet too short"
    ...
    if version <> 2
    then Error "RTCP version mismatch"
    else if len < total_len
    then Error "RTCP packet truncated"
    else (
      let padding_len = if not padding then 0 else Bytes.get_uint8 data (total_len - 1) in
      let payload_len = total_len - padding_len in
      if padding && (padding_len = 0 || padding_len > total_len - 4)
      then Error "RTCP padding invalid"
      else (
        ...
        | SR ->
          if body_len < 24 + (count * 24)
          then Error "RTCP SR body too short"
          else (
        ...
        | RR ->
          if body_len < 4 + (count * 24)
          then Error "RTCP RR body too short"
          else (
            let ssrc = read_uint32_be data body_off in
        ...
        | SDES ->
          ...
          let rec loop idx off acc =
            if idx = count
            then Ok (Source_description (List.rev acc))
            else (
        ...
        | BYE ->
          let ssrc_len = count * 4 in
          if body_len < ssrc_len
          then Error "RTCP BYE body too short"
          else (
            let ssrcs = ref [] in
            ...
            let reason =
              ...
              if reason_off >= body_off + body_len
              then None
              else (
                let len = Bytes.get_uint8 data reason_off in
                if reason_off + 1 + len > body_off + body_len
                then None (* Malformed, but be lenient *)
                else Some (Bytes.sub_string data (reason_off + 1) len))
            in
            Ok (Bye { ssrcs = List.rev !ssrcs; reason }))
        | APP ->
          if body_len < 8
          then Error "RTCP APP body too short"
          else (
            let ssrc = read_uint32_be data body_off in
...
let decode_compound data =
  ...
  let rec loop off acc =
    ...
    then Error "RTCP compound truncated"
    else (
      let length_words = read_uint16_be data (off + 2) in
      let total_len = (length_words + 1) * 4 in
      if off + total_len > len
      then Error "RTCP compound length invalid"
      else (
...
let make_sdes_cname ~ssrc ~cname =
  Source_description [ { ssrc; items = [ { item_type = CNAME; value = cname } ] } ]
;;

(** Helper: Create BYE packet. *)
let make_bye ?reason ssrcs = Bye { ssrcs; reason }
...
let calculate_rtcp_interval ~members ~senders ~rtcp_bw ~we_sent ~avg_rtcp_size ~initial =
  ...
  let n, c =
    ...
    then
      ...
      if we_sent then senders, rtcp_bw *. 0.25 else members -. senders, rtcp_bw *. 0.75
    else
      (* Senders > 25%: share bandwidth equally *)
      members, rtcp_bw
  in
  (* Calculate interval: (avg_rtcp_size * n) / c *)
  let interval = if c <= 0.0 then 5.0 else avg_rtcp_size *. n /. c in
  (* Apply minimum *)
