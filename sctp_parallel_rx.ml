...
let stats_key : domain_stats Domain.DLS.key =
  Domain.DLS.new_key create_domain_stats
...
type rx_output =
  | DeliverData of { stream_id: int; data: bytes }
  | SendSack of bytes
  | Error of string
...
let create ?(num_domains = max 1 (Domain.recommended_domain_count () / 2)) () =
  ...
  let domains = Array.init num_domains (fun id ->
    create_rx_domain ~id ~output_queue
  ) in
  {
    domains;
    output_queue;
...
let drain_outputs t =
  Mpsc_queue.drain t.output_queue
...
let is_running t =
  Array.exists (fun d -> d.running) t.domains

(** Pretty print stats *)
let pp_stats fmt (s : aggregated_stats) =
  Format.fprintf fmt "parallel_rx: domains=%d pkts=%d bytes=%d msgs=%d errs=%d"
    s.domain_count
