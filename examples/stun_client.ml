...
let discover_public_ip ~server_host ~server_port =
  ...
  let server_addr =
    ...
    with
    | Not_found ->
      Log.error "Error: Cannot resolve hostname %s" server_host;
      exit 1
  in
  ...
  let hex_tid =
    String.concat
      ""
      (List.init (Bytes.length tid) (fun i ->
  ...
  try
        ...
        (fun attr ->
           ...
           | Stun.Mapped_address addr ->
             Printf.printf
               "│   Mapped Address: %-28s │\n"
               (Printf.sprintf "%s:%d" addr.Stun.ip addr.Stun.port)
           | Stun.Software name -> Printf.printf "│   Server Software: %-27s │\n" name
           | _ -> ())
    ...
    | Error e ->
      ...
      1
  with
  | Unix.Unix_error (Unix.ETIMEDOUT, _, _) | Unix.Unix_error (Unix.EAGAIN, _, _) ->
    Log.error "Error: Request timed out";
    Unix.close sock;
    1
;;
...
let () =
  let server_host =
    if Array.length Sys.argv > 1 then Sys.argv.(1) else default_stun_server
  in
  let server_port =
    if Array.length Sys.argv > 2 then int_of_string Sys.argv.(2) else default_stun_port
  in
