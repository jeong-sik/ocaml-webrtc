...
let run_server port =
  ...
  while !received < 3 do
    ...
    let data = Bytes.sub buf 0 len in
    if is_dtls data then
      Printf.printf "Ignoring DTLS packet after handshake.\n%!"
    else if is_rtcp data then
      Printf.printf "Server received SRTCP (unexpected).\n%!"
    else
  ...
  begin match Media_transport.protect_rtcp media ~packet:rr () with
  | Ok srtcp ->
    ignore (ops.send srtcp);
    Printf.printf "Sent SRTCP RR.\n%!"
  | Error e ->
    Printf.eprintf "SRTCP error: %s\n%!" e
  end;
