...
let is_rtp_or_rtcp data =
  Bytes.length data > 0 && (Bytes.get_uint8 data 0 land 0xC0) = 0x80

let is_rtcp data =
  Bytes.length data > 1 && Bytes.get_uint8 data 1 >= 192

let advance_rtcp_index idx =
  Int32.logand (Int32.succ idx) 0x7FFFFFFFl
...
let () =
  ...
  let specs = [
    ...
    ("--public-ip", Arg.String (fun v -> advertise_ip := Some v), "Public IP for SDP candidate");
    ...
    ("--ssrc", Arg.Int (fun v -> ssrc_override := Some (Int32.of_int v)), "Local SSRC (int)");
  ] in
  ...
  let public_ip =
    ...
    | None ->
      if !listen_ip = "0.0.0.0" then
        failwith "--public-ip required when listen-ip is 0.0.0.0"
      else
        !listen_ip
  in
  ...
  let answer = Sdp.create_answer ~offer ~ice_ufrag:local_ufrag ~ice_pwd:local_pwd ~fingerprint in
  let answer = { answer with
    ice_lite = true;
  ...
  let candidate = {
    Ice.foundation = Ice.generate_foundation ~candidate_type:Ice.Host ~base_address:public_ip ();
    component = 1;
    transport = Ice.UDP;
    priority = Ice.calculate_priority ~candidate_type:Ice.Host ~local_pref:65535 ~component:1;
    address = public_ip;
    port = !listen_port;
    cand_type = Ice.Host;
    base_address = None;
  ...
  let set_peer addr =
    if !peer_addr = None then peer_addr := Some addr
  in
  ...
  let send_bytes data =
    ...
    | None -> ()
    | Some addr ->
      ignore (Unix.sendto sock data 0 (Bytes.length data) [] addr)
  in

  let send_dtls records =
    List.iter send_bytes records
  in
  ...
  let send_rtcp_report () =
    ...
    | Some keys, Some r_ssrc ->
      ...
      let rr = Rtcp.Receiver_report {
        ssrc = local_ssrc;
        report_blocks = [report_block];
      } in
      let sdes = Rtcp.Source_description [{
        ssrc = local_ssrc;
        items = [{ item_type = Rtcp.CNAME; value = !cname }];
      }] in
      let compound = Bytes.concat Bytes.empty [Rtcp.encode rr; Rtcp.encode sdes] in
      begin match Srtp.protect_rtcp ~profile:Srtp.SRTP_AES128_CM_HMAC_SHA1_80
        ~keys ~index:!rtcp_index ~encrypt:true ~packet:compound with
      | Ok protected ->
        rtcp_index := advance_rtcp_index !rtcp_index;
        send_bytes protected
      | Error e ->
        Printf.eprintf "[RTCP] protect failed: %s\n%!" e
      end
    | _ -> ()
  in
  ...
  let handle_stun msg addr =
    ...
    | Stun.Request, Stun.Binding ->
      let has_integrity =
        List.exists (fun attr -> attr.Stun.attr_type = Stun.MESSAGE_INTEGRITY) msg.attributes
      in
      let integrity_ok = (not has_integrity) || Stun.verify_integrity msg ~key:local_pwd in
      if integrity_ok then begin
        let (ip, port) =
          match addr with
  ...
  let handle_dtls data addr =
    ...
    | Ok (records, _app) ->
      send_dtls records;
      if Dtls.is_established dtls && !media_state = None then begin
        match Dtls_srtp.session_keys_of_dtls
          ~dtls ~role:Dtls_srtp.Server ~profile:Srtp.SRTP_AES128_CM_HMAC_SHA1_80 () with
        | Ok (local, remote) ->
          local_keys := Some local;
          remote_keys := Some remote;
          media_state := Some (Media_transport.create
            ~profile:Srtp.SRTP_AES128_CM_HMAC_SHA1_80
            ~local_keys:local ~remote_keys:remote
            ~ssrc:local_ssrc ~payload_type:!payload_type);
          Printf.printf "[DTLS] established, SRTP ready\n%!"
        | Error e ->
          Printf.eprintf "[DTLS] SRTP key export failed: %s\n%!" e
      end
    | Error e ->
      Printf.eprintf "[DTLS] error: %s\n%!" e
  in
  ...
  let handle_rtp data =
    ...
    | Some media ->
      ...
      | Ok pkt ->
        ...
        if not !rtcp_sent then begin
          send_rtcp_report ();
          rtcp_sent := true
        end
      | Error e ->
        Printf.eprintf "[RTP] unprotect failed: %s\n%!" e
  in
  ...
  let handle_rtcp data =
    ...
    | None -> ()
    | Some keys ->
      match Srtp.unprotect_rtcp ~profile:Srtp.SRTP_AES128_CM_HMAC_SHA1_80 ~keys ~packet:data with
      | Ok (plaintext, _index) ->
        begin match Rtcp.decode_compound plaintext with
        | Ok _ -> ()
        | Error e -> Printf.eprintf "[RTCP] decode failed: %s\n%!" e
        end;
        if not !rtcp_sent then begin
          send_rtcp_report ();
          rtcp_sent := true
      ...
      | Error e ->
        Printf.eprintf "[RTCP] unprotect failed: %s\n%!" e
  in
  ...
  while true do
    ...
    if Stun.is_stun_message data then
      match Stun.decode data with
      | Ok msg -> handle_stun msg addr
      | Error _ -> ()
    else if is_dtls_packet data || not (Dtls.is_established dtls) then
      handle_dtls data addr
    else if is_rtp_or_rtcp data then
      if is_rtcp data then handle_rtcp data else handle_rtp data
    else
      ()
  done
