...
let parse_error_response data =
  if Bytes.length data < 20 then
    ...
    if msg_class <> 0x0110 then (* Not error response *)
      None
    else begin
      let rec scan offset realm nonce error_code =
        if offset >= Bytes.length data - 4 then
          (realm, nonce, error_code)
        else
          ...
          let result =
            if atype = turn_attr_to_int REALM && Bytes.length data >= value_start + alen then
              (Some (Bytes.sub_string data value_start alen), nonce, error_code)
            else if atype = turn_attr_to_int NONCE && Bytes.length data >= value_start + alen then
              (realm, Some (Bytes.sub_string data value_start alen), error_code)
            else if atype = turn_attr_to_int ERROR_CODE && Bytes.length data >= value_start + 4 then
              ...
              (realm, nonce, error_code)
          in
          scan (offset + 4 + padded) (match result with (r, _, _) -> r)
                                     (match result with (_, n, _) -> n)
                                     (match result with (_, _, e) -> e)
      in
...
let build_authenticated_request t msg_type attrs tid =
  ...
  let username_attr = build_attribute (turn_attr_to_int USERNAME)
    (Bytes.of_string t.config.username) in
  (* Build REALM attribute *)
  let realm_attr = build_attribute (turn_attr_to_int REALM)
    (Bytes.of_string t.config.realm) in
  (* Build NONCE attribute *)
  let nonce_attr = build_attribute (turn_attr_to_int NONCE)
    (Bytes.of_string t.nonce) in
  (* Combine all attributes except MESSAGE-INTEGRITY *)
  let all_attrs = Bytes.concat Bytes.empty
    [attrs; username_attr; realm_attr; nonce_attr] in
...
let build_allocate_request t =
  ...
  let transport_attr = build_attribute (turn_attr_to_int REQUESTED_TRANSPORT) transport_buf in
  (* LIFETIME attribute *)
...
let build_create_permission_request peer_ip =
  ...
  Bytes.set_uint8 addr_buf 0 0;  (* Reserved *)
  Bytes.set_uint8 addr_buf 1 1;  (* IPv4 *)
  Bytes.set_uint16_be addr_buf 2 0;  (* Port XORed *)
  (* Parse IP and XOR with magic cookie *)
  let parts = String.split_on_char '.' peer_ip in
  (match parts with
  | [a; b; c; d] ->
    let xored = Int32.logxor magic_cookie
      (Int32.of_int ((int_of_string a lsl 24) lor (int_of_string b lsl 16) lor
                     (int_of_string c lsl 8) lor (int_of_string d))) in
...
let parse_response data =
  if Bytes.length data < 20 then
    ...
    let msg_class = msg_type land 0x0110 in
    Result.Ok (msg_class = 0x0100)  (* Success response *)

let parse_xor_relayed_address data offset tid =
  if Bytes.length data < offset + 8 then
    None
  else
    ...
    let _ = tid in  (* tid used for IPv6 XOR *)
    let ip = Printf.sprintf "%ld.%ld.%ld.%ld"
      ...
      (Int32.logand addr 0xFFl) in
    Some (ip, port)

let find_attribute data attr_type =
  let rec scan offset =
    if offset >= Bytes.length data - 4 then None
    else
      let atype = Bytes.get_uint16_be data offset in
      let alen = Bytes.get_uint16_be data (offset + 2) in
      if atype = attr_type then
        Some (offset + 4, alen)
      else
        let padded = (alen + 3) land (lnot 3) in
...
let process_allocation_success t response tid =
  ...
  let relayed = match find_attribute response (turn_attr_to_int XOR_RELAYED_ADDRESS) with
    | Some (offset, _len) -> parse_xor_relayed_address response offset tid
    | None -> None
  in
  (* Extract LIFETIME *)
  let lifetime = match find_attribute response (turn_attr_to_int LIFETIME) with
...
let allocate t =
  ...
  | Result.Ok false ->
    ...
    begin match parse_error_response response with
    | Some (server_realm, server_nonce, 401) ->
      ...
      let _ = server_realm in  (* Use server realm if needed in future *)
      (* Build authenticated request *)
      let transport_attr = build_attribute (turn_attr_to_int REQUESTED_TRANSPORT)
        (Bytes.make 4 (Char.chr 17)) in  (* UDP = 17 *)
      let auth_request = build_authenticated_request t (turn_method_to_int Allocate) transport_attr tid in
      let _ = send_to_server t auth_request in
      let (auth_response, _, _) = recv_from_server () in
      begin match parse_response auth_response with
  ...
  | Result.Ok true ->
    process_allocation_success t response tid
...
let refresh t ?(lifetime = t.config.lifetime) () =
  ...
  | Active old_state ->
    ...
    (match parse_response response with
    | Result.Ok true ->
      let new_lifetime = match find_attribute response (turn_attr_to_int LIFETIME) with
        | Some (offset, _) when Bytes.length response >= offset + 4 ->
          Int32.to_int (Bytes.get_int32_be response offset)
        | _ -> lifetime
      in
  ...
  | _ ->
    Result.Error "No active allocation"
...
let create_permission t peer_ip =
  ...
  | Result.Ok true ->
    ...
    Result.Ok ()
  | _ ->
    Result.Error "CreatePermission failed"

let channel_bind t number peer_addr =
  if number < 0x4000 || number > 0x7FFF then
    Result.Error "Channel number must be in range 0x4000-0x7FFF"
  else begin
    ...
    | Result.Ok true ->
      ...
      Result.Ok ()
    | _ ->
      Result.Error "ChannelBind failed"
  end
...
let on_data t callback =
  t.on_data_callback <- Some callback
...
let pp_state fmt = function
  ...
  | Active a ->
    Format.fprintf fmt "Active(relay=%s:%d, lifetime=%d)"
      (fst a.relayed_address) (snd a.relayed_address) a.lifetime
...
let run_with_eio ~net ~clock f =
  try_with f () {
    effc = fun (type a) (eff : a Effect.t) ->
      match eff with
      | Send (data, host, port) -> Some (fun (k : (a, _) continuation) ->
      ...
      | Sleep secs -> Some (fun (k : (a, _) continuation) ->
          let _ = (secs, clock) in
          continue k ()
        )
