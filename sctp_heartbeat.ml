...
let decode_heartbeat_info buf =
  ...
  then Error "Heartbeat info too short"
  else (
...
let encode_heartbeat info =
  ...
  (* Parameter type: Heartbeat Info *)
  Bytes.set_int16_be buf 6 20;
  (* Parameter length: 4 + 16 *)
...
let decode_heartbeat buf =
  if Bytes.length buf < 24
  then Error "HEARTBEAT chunk too short"
  else (
    let chunk_type = Char.code (Bytes.get buf 0) in
    if chunk_type <> chunk_type_heartbeat
    then Error "Not a HEARTBEAT chunk"
    else (
...
let decode_heartbeat_ack buf =
  if Bytes.length buf < 24
  then Error "HEARTBEAT-ACK chunk too short"
  else (
    let chunk_type = Char.code (Bytes.get buf 0) in
    if chunk_type <> chunk_type_heartbeat_ack
    then Error "Not a HEARTBEAT-ACK chunk"
    else (
      let info_buf = Bytes.sub buf 8 16 in
      decode_heartbeat_info info_buf))
;;
...
let is_path_down t = t.consecutive_failures >= t.max_failures
...
let update_rto ~current_srtt ~current_rttvar ~measured_rtt =
  ...
  then (
    ...
    let rttvar =
      ((1.0 -. beta) *. current_rttvar)
      +. (beta *. abs_float (current_srtt -. measured_rtt))
    in
...
let pp fmt t =
  ...
  Format.fprintf
    fmt
    "Heartbeat{failures=%d/%d, pending=%s}"
