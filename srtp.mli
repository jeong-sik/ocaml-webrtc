...
val srtp_iv : salt:bytes -> ssrc:int32 -> index:int64 -> (bytes, string) result

(** Encrypt/decrypt payload using AES-CM (RFC 3711 §4.1.1). *)
val aes_cm_crypt : key:bytes -> iv:bytes -> payload:bytes -> (bytes, string) result

(** Compute SRTP authentication tag (RFC 3711 §4.2.1).
    [packet] should be the RTP header + encrypted payload. *)
val srtp_auth_tag : auth_key:bytes -> packet:bytes -> roc:int32 -> tag_len:int -> bytes

(** Compute SRTCP authentication tag (RFC 3711 §4.2.1).
    [packet] should include the SRTCP index field (with E bit). *)
val srtcp_auth_tag : auth_key:bytes -> packet:bytes -> tag_len:int -> bytes
...
val srtp_gcm_iv : salt:bytes -> ssrc:int32 -> index:int64 -> (bytes, string) result

(** Build SRTCP AES-GCM IV/nonce (RFC 7714 Section 9.1).
    12 bytes: salt XOR (SSRC || 00 || SRTCP index) *)
val srtcp_gcm_iv : salt:bytes -> ssrc:int32 -> index:int32 -> (bytes, string) result
