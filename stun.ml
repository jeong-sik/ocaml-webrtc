...
let get_uint32_be buf off =
  Int32.(
    logor
      (shift_left (of_int (Char.code (Bytes.get buf off))) 24)
      (logor
         (shift_left (of_int (Char.code (Bytes.get buf (off + 1)))) 16)
         (logor
            (shift_left (of_int (Char.code (Bytes.get buf (off + 2)))) 8)
...
let set_uint32_be buf off v =
  ...
  Bytes.set
    buf
    (off + 1)
    (Char.chr Int32.(to_int (logand (shift_right_logical v 16) 0xFFl)));
  Bytes.set
    buf
    (off + 2)
    (Char.chr Int32.(to_int (logand (shift_right_logical v 8) 0xFFl)));
...
let encode_ipv4 ip =
  ...
  List.iteri (fun i part -> Bytes.set bytes i (Char.chr (int_of_string part))) parts;
  bytes
;;
...
let decode_address data off =
  ...
  | _ -> Error (Printf.sprintf "Unknown address family: 0x%02X" family_byte)
;;
...
let xor_address addr transaction_id =
  ...
  | IPv4 ->
    ...
    let xored_port =
      addr.port lxor Int32.to_int (Int32.shift_right_logical magic_cookie 16)
    in
    ...
    for i = 0 to 3 do
      Bytes.set
        ...
        i
        (Char.chr
           (Char.code (Bytes.get ip_bytes i) lxor Char.code (Bytes.get mc_bytes i)))
    done;
  ...
  | IPv6 ->
    ...
    for i = 0 to 15 do
      Bytes.set
        ...
        i
        (Char.chr
           (Char.code (Bytes.get ip_bytes i) lxor Char.code (Bytes.get xor_mask i)))
    done;
...
let encode_attribute attr =
  ...
  let value_bytes =
    ...
    | Xor_relayed_address addr -> encode_address addr
    | Username s | Software s | Realm s | Nonce s -> Bytes.of_string s
    | Message_integrity b -> b
    | Fingerprint fp ->
    ...
    | Error_code { code; reason } ->
      ...
      Bytes.set b 0 '\x00';
      Bytes.set b 1 '\x00';
      Bytes.set b 2 (Char.chr (code / 100));
      (* Class *)
      Bytes.set b 3 (Char.chr (code mod 100));
      (* Number *)
    ...
    | Even_port reserve ->
      ...
      b
    | Dont_fragment -> Bytes.empty (* Zero-length attribute *)
    | Reservation_token token -> token (* 8 bytes *)
    | Unknown_attr b -> b
  in
...
let decode_attribute data off =
  if Bytes.length data < off + 4
  then Error "Attribute too short"
  else (
    let type_code = get_uint16_be data off in
    let value_len = get_uint16_be data (off + 2) in
    let padded_len = (value_len + 3) land lnot 3 in
    if Bytes.length data < off + 4 + padded_len
    then Error "Attribute value truncated"
    else (
      ...
      let value =
        ...
        | ERROR_CODE ->
          ...
          let reason =
            if value_len > 4 then Bytes.sub_string value_data 4 (value_len - 4) else ""
          in
          Error_code { code; reason }
        (* RFC 5766 TURN attributes *)
        | CHANNEL_NUMBER ->
          if Bytes.length value_data >= 2
          then Channel_number (get_uint16_be value_data 0)
          else Unknown_attr value_data
        | LIFETIME ->
          if Bytes.length value_data >= 4
          then Lifetime (Int32.to_int (get_uint32_be value_data 0))
          else Unknown_attr value_data
        | DATA -> Data_attr value_data
        | REQUESTED_TRANSPORT ->
          if Bytes.length value_data >= 1
          then Requested_transport (Char.code (Bytes.get value_data 0))
          else Unknown_attr value_data
        | EVEN_PORT ->
          if Bytes.length value_data >= 1
          then Even_port (Char.code (Bytes.get value_data 0) land 0x80 <> 0)
          else Unknown_attr value_data
        | DONT_FRAGMENT -> Dont_fragment
        | RESERVATION_TOKEN -> Reservation_token value_data
...
let decode data =
  ...
  then Error "Message too short (< 20 bytes)"
  else (
    (* Check first two bits are 0 *)
    let first_byte = Char.code (Bytes.get data 0) in
    if first_byte land 0xC0 <> 0
    then Error "Invalid STUN message: first two bits must be 0"
    else (
      (* Check magic cookie *)
      ...
      then
        Error
          (Printf.sprintf "Invalid magic cookie: 0x%08lX (expected 0x2112A442)" cookie)
      else (
        let msg_type = get_uint16_be data 0 in
        let msg_len = get_uint16_be data 2 in
        if Bytes.length data < 20 + msg_len
        then Error "Message truncated"
        else (
          ...
          | Ok attributes -> Ok { msg_class; msg_method; transaction_id; attributes }
          | Error e -> Error e))))
;;
...
let allocate_request_lwt
  ...
  | Some (host, port) ->
    let decode_response request result =
      ...
      | Error e -> Error e
      | Ok response ->
        if not (Bytes.equal response.transaction_id request.transaction_id)
    ...
    let handle_success response =
      ...
      | Error e -> Error e
    in
    let handle_success_lwt response = Lwt.return (handle_success response) in
    ...
    in
    let rec send_authenticated_lwt
              send_and_recv
              ~username
              ~password
              ~realm
      ...
      send_and_recv auth_request
      >>= function
      | Error e -> Lwt.return (Error e)
      | Ok response ->
        if response.msg_class = Error_response
        then (
          ...
          | Some 438 when attempts > 0 ->
            ...
            send_authenticated_lwt
              ...
              ~nonce:next_nonce
              ~attempts:(attempts - 1)
          | Some code -> Lwt.return (Error (Printf.sprintf "TURN error %d" code))
          | None -> Lwt.return (Error "TURN error response (unknown error)"))
        else handle_success_lwt response
    in
    let rec send_authenticated_blocking
              send_and_recv
              ~username
              ~password
              ~realm
        ...
        then (
          ...
          | Some 438 when attempts > 0 ->
            ...
            send_authenticated_blocking
              ...
              ~nonce:next_nonce
              ~attempts:(attempts - 1)
          | Some code -> Error (Printf.sprintf "TURN error %d" code)
          | None -> Error "TURN error response (unknown error)")
        else handle_success response
    in
    let send_and_recv_udp sock addr request =
      ...
      let recv_promise =
        ...
        >>= fun (len, _from) -> Lwt.return (Bytes.sub response_buf 0 len)
      in
      Lwt.pick
        [ recv_promise
        ; (Lwt_unix.sleep timeout_s >>= fun () -> Lwt.fail (Failure "Timeout"))
        ]
      >>= fun result -> decode_response_lwt request result
    in
          ...
          (fun () ->
             ...
             | Ok response ->
               ...
               then (
                 match find_error_code response with
                 | Some 401 | Some 438 ->
                   (match username, credential with
                    | Some username, Some password ->
                      let realm_opt, nonce_opt = find_realm_nonce response in
                      (match realm_opt, nonce_opt with
                       | Some realm, Some nonce ->
                         send_authenticated_lwt
                           ...
                           ~nonce
                           ~attempts:1
                       | _ -> Lwt.return (Error "TURN auth missing realm/nonce"))
                    | _ ->
      ...
      | Ok authenticator ->
        (match connect_tcp_with_timeout ~host ~port ~timeout_s with
         ...
         | Ok sock ->
           ...
           (match tls_client_config ~authenticator host with
            | Error msg ->
              Unix.close sock;
              Error msg
            | Ok (config, host_name) ->
                 ...
                 Fun.protect
                   ...
                   (fun () ->
                      let send_and_recv request =
                        ...
                        | Error e -> Error e
                        | Ok () ->
                          (match read_stun_frame_tls tls with
                               ...
                               (match realm_opt, nonce_opt with
                                | Some realm, Some nonce ->
                                  send_authenticated_blocking
                                    ...
                                    ~nonce
                                    ~attempts:1
                                | _ -> Error "TURN auth missing realm/nonce")
                             | _ -> Error "Unauthorized (401): Authentication required")
                          | Some code -> Error (Printf.sprintf "TURN error %d" code)
                          | None -> Error "TURN error response (unknown error)")
                        else handle_success response))))
    in
    let allocate_over_tls () =
      Lwt_preemptive.detach
        ...
        ()
    in
    if use_tls then allocate_over_tls () else allocate_over_udp ()
;;
  ...
  | Some (host, port) ->
    ...
    (match addrs with
     ...
     | addr :: _ ->
       ...
       let response_buf = Bytes.create 576 in
       (* Max STUN message size *)
       let recv_promise =
       ...
       (match decode result with
        ...
        | Ok response ->
          ...
          if not (Bytes.equal response.transaction_id request.transaction_id)
          then Lwt.return (Error "Transaction ID mismatch")
          else if response.msg_class = Error_response
          then Lwt.return (Error "STUN error response")
          else (
            ...
            let rec find_addr = function
              ...
              | { value = Mapped_address addr; _ } :: _ -> Some addr
            ...
            | Some mapped_address ->
              ...
              let local_address =
                ...
                | Unix.ADDR_UNIX _ -> { family = IPv4; port = 0; ip = "0.0.0.0" }
              in
              Lwt.return
                (Ok
                   { local_address
