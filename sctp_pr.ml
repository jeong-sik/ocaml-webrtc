...
let mark_sent t ~tsn =
  t.messages <- List.map (fun m ->
    if m.tsn = tsn && m.state = Pending then
      { m with state = InFlight }
    else m
  ) t.messages

(** Mark message as delivered (ACKed) *)
let mark_delivered t ~tsn =
  t.messages <- List.filter (fun m -> m.tsn <> tsn) t.messages

(** Increment retransmission count *)
let record_retransmit t ~tsn =
  t.messages <- List.map (fun m ->
    if m.tsn = tsn then
      { m with rtx_count = m.rtx_count + 1 }
    else m
  ) t.messages
...
let should_abandon msg =
  ...
  | LimitedRetransmit { max_rtx } ->
    msg.rtx_count >= max_rtx
...
let check_abandonments t =
  ...
  t.messages <- List.map (fun m ->
    if m.state = InFlight && should_abandon m then begin
...
let generate_forward_tsn t ~current_cumulative_tsn =
  if t.abandoned_tsns = [] then None
  else begin
    (* Find the highest TSN we can advance to *)
    let abandoned_sorted = List.sort Int32.compare t.abandoned_tsns in
    let new_cum_tsn = List.fold_left (fun acc tsn ->
      if Int32.succ acc = tsn then tsn else acc
    ) current_cumulative_tsn abandoned_sorted in

    (* Get stream sequences for abandoned messages *)
    let stream_seqs = List.filter_map (fun m ->
      if m.state = Abandoned && m.tsn <= new_cum_tsn then
        Some { stream_id = m.stream_id; stream_seq = m.stream_seq + 1 }
      else None
    ) t.messages in
    ...
    t.messages <- List.filter (fun m ->
      m.state <> Abandoned || m.tsn > new_cum_tsn
    ) t.messages;

    Some {
      new_cumulative_tsn = new_cum_tsn;
      stream_seqs;
    }
  end
...
let encode_forward_tsn ftsn =
  ...
  (* Stream sequences *)
  List.iteri (fun i (ss : stream_seq) ->
    let off = 8 + i * 4 in
...
let decode_forward_tsn buf =
  if Bytes.length buf < 8 then Error "Forward-TSN too short"
  else begin
    let chunk_type = Bytes.get buf 0 |> Char.code in
    if chunk_type <> chunk_type_forward_tsn then
      Error (Printf.sprintf "Not Forward-TSN chunk: %d" chunk_type)
    else begin
      ...
      let stream_seqs = List.init num_streams (fun i ->
        let off = 8 + i * 4 in
        {
...
let process_forward_tsn ~recv_cumulative_tsn ftsn =
  ...
  if ftsn.new_cumulative_tsn > recv_cumulative_tsn then
    ftsn.new_cumulative_tsn
  else
    recv_cumulative_tsn
...
let string_of_policy = function
  ...
  | LimitedRetransmit { max_rtx } ->
    Printf.sprintf "LimitedRetransmit(%d)" max_rtx
