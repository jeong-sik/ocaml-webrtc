...
let classify msg =
  if matches_any fatal_patterns msg then Fatal
  else if matches_any transient_patterns msg then Transient
  else if matches_any config_patterns msg then Config
  else Protocol
  ...
  | Some i when i < 30 ->
    ...
    if String.length prefix > 0 then
      ...
      | Some sp ->
        ...
        if String.length first_word > 0
           && Char.uppercase_ascii first_word.[0] = first_word.[0]
        then first_word
        else "unknown"
      | None ->
        if Char.uppercase_ascii prefix.[0] = prefix.[0]
        then prefix
        else "unknown"
    else "unknown"
  | _ ->
    (* Try to match known module patterns *)
    if string_contains ~sub:"CRC32c" msg then "sctp_core"
    else if string_contains ~sub:"Congestion" msg then "sctp_core"
    else if string_contains ~sub:"HEARTBEAT" msg then "sctp_heartbeat"
    else if string_contains ~sub:"Shutdown" msg then "sctp_core"
    else if string_contains ~sub:"Handshake" msg then "dtls"
    else if string_contains ~sub:"DATA_CHANNEL" msg then "dcep"
    else if string_contains ~sub:"RTP" msg then "media_transport"
    else if string_contains ~sub:"Packet too short" msg then "sctp_core"
    else if string_contains ~sub:"Association" msg then "sctp_core"
    else "unknown"

let of_string msg =
  { cls = classify msg
  ; message = msg
  ; module_hint = infer_module msg
  }
