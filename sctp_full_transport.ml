...
let create ?(config = Sctp.default_config) ?initial_tsn ~host ~port () =
  ...
  ; send_buffer = Bytes.create 65536
...
let send_packet t packet = Udp_transport.send_connected t.udp ~data:packet
...
let bundle_chunk t chunk =
  ...
  if t.bundle_offset + padded_len > t.config.mtu then flush_bundle t;
  (* Encode chunk into bundle buffer *)
  let encoded_len =
    Sctp.encode_data_chunk_into ~buf:t.send_buffer ~off:t.bundle_offset chunk
  in
...
let process_single_chunk t ~buf ~off ~remaining =
  if remaining < 4
  then remaining, None (* Not enough for header *)
  else (
    ...
    then remaining, None (* Incomplete chunk *)
    else (
      match chunk_type with
      | 0 ->
        (* DATA chunk *)
        (match Sctp.decode_data_chunk_view buf ~off ~len:padded_len with
         | Ok chunk ->
      ...
      | 3 ->
        (* SACK chunk *)
        (match Sctp_reliable.decode_sack (Bytes.sub buf off padded_len) with
         | Ok sack ->
...
let process_packet_view t ~buf ~off ~len =
  ...
  while !remaining > 0 do
    let consumed, data_opt =
      process_single_chunk t ~buf ~off:!pos ~remaining:!remaining
    in
    pos := !pos + consumed;
  ...
  if !chunks_processed > 0 && t.packets_since_sack >= 2
  then (
  ...
  else if !chunks_processed > 0
  then t.pending_sack <- true;
  !last_data
;;
...
let try_recv_data t =
  ...
  | None -> None
  | Some packet -> process_packet t packet
;;
...
let tick t =
  ...
  if !packets_processed = 0 then flush_pending_sack t;
...
let pp_cc_state fmt t = Sctp_reliable.pp_cc_state fmt t.reliable
