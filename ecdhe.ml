...
let generate_p256 () : (keypair, string) result =
  ...
  with
  | exn -> Error (Printf.sprintf "P-256 key generation failed: %s" (Printexc.to_string exn))
...
let generate_x25519 () : (keypair, string) result =
  ...
  with
  | exn -> Error (Printf.sprintf "X25519 key generation failed: %s" (Printexc.to_string exn))
...
let compute_shared_secret_x25519 ~private_key ~peer_public_key : (Cstruct.t, string) result =
  let priv_str = Cstruct.to_string private_key in
  let peer_pub_str = Cstruct.to_string peer_public_key in
...
let compute_shared_secret ~keypair ~peer_public_key : (Cstruct.t, string) result =
  ...
  | Secp256r1 -> compute_shared_secret_p256 ~private_key:keypair.private_key ~peer_public_key
  | X25519 -> compute_shared_secret_x25519 ~private_key:keypair.private_key ~peer_public_key
  | Secp384r1 -> Error "P-384 not yet implemented"
  | Secp521r1 -> Error "P-521 not yet implemented"
...
let decode_public_key ~curve data : (Cstruct.t, string) result =
  if Cstruct.length data < 1 then
    ...
    else if Cstruct.length data < 1 + len then
      Error "Public key data truncated"
    else
      Ok (Cstruct.sub data 1 len)
...
let build_server_ecdh_params keypair =
  ...
  Cstruct.set_uint8 curve_params 0 3;  (* named_curve type *)
...
let parse_server_ecdh_params data : (named_curve * Cstruct.t, string) result =
  if Cstruct.length data < 4 then
    Error "ServerECDHParams too short"
  else
    let curve_type = Cstruct.get_uint8 data 0 in
    if curve_type <> 3 then
      Error (Printf.sprintf "Unsupported curve type: %d (expected named_curve=3)" curve_type)
    else
...
let compute_verify_data ~master_secret ~handshake_hash ~is_client =
  ...
  let verify_data_length = 12 in  (* RFC 5246: always 12 bytes *)
...
let pp_named_curve fmt curve =
  Format.fprintf fmt "%s" (string_of_named_curve curve)

let pp_keypair fmt kp =
  Format.fprintf fmt "ECDHE(%s, pub=%d bytes)"
