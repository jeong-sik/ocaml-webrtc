      ...
      let rec wait () =
        ...
        then None
          ...
          | Some response when Stun.is_stun_message response ->
            (match Stun.decode response with
             | Ok msg ->
               (* Extract XOR-MAPPED-ADDRESS *)
               let mapped =
                 List.find_opt
                   (fun (attr : Stun.attribute) ->
...
let start_gathering t ~clock:_ =
  ...
  (match host_candidates with
   | cand :: _ ->
     let fd = create_socket ~host:cand.address ~port:cand.port in
     t.socket <- Some fd
   | [] -> ());
  (* Notify each host candidate *)
  List.iter (fun cand -> Option.iter (fun f -> f cand) t.on_candidate) host_candidates;
            ...
            | cand :: _ ->
              (match
                 gather_srflx
                   t
                   ~stun_server:url
                   ~local_addr:cand.address
                   ~local_port:cand.port
               with
               | Some srflx ->
...
let add_remote_candidate t candidate = Ice.add_remote_candidate t.agent candidate
let set_remote_credentials t ~ufrag ~pwd = Ice.set_remote_credentials t.agent ~ufrag ~pwd
...
let try_recv t =
  match t.socket with
  | Some fd ->
    (match recv_udp fd ~buf:t.recv_buffer with
     | Some data ->
       (* Demux: STUN or application data *)
       if Bytes.length data > 0 && Char.code (Bytes.get data 0) land 0xC0 = 0
       then
         (* STUN message *)
         None
       else Some data
