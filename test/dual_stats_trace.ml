...
let () =
  ...
  Printf.printf
    "  reliable.stats: fast_rtx=%d timeouts=%d retx=%d\n"
    rel_stats0.fast_retransmissions
    rel_stats0.timeouts
    rel_stats0.retransmissions;
  Printf.printf
    "  transport.stats: fast_rtx=%d retx=%d\n\n%!"
    tr_stats0.fast_retransmissions
    tr_stats0.retransmissions;
  ...
  for step = 31701 to 31850 do
    ...
    if tick_pkts > 0 || rel_fast_delta > 0 || rel_timeout_delta > 0 || step mod 20 = 0
    then
      Printf.printf
        "step %d: send:%d tick:%d | rel_mark:%d rel_to:%d | tr_fast:%d tr_rtx:%d | \
         flight:%d->%d\n\
         %!"
        step
        send_pkts
    ...
    then (
      ...
      Printf.printf
        "Final reliable stats: fast=%d to=%d rtx=%d\n"
        rel_post.fast_retransmissions
        rel_post.timeouts
        rel_post.retransmissions;
      Printf.printf
        "Final transport stats: fast=%d rtx=%d\n"
        tr_post.fast_retransmissions
        tr_post.retransmissions;
      exit 0)
  done;
  Printf.printf "\nFinal stats:\n";
  ...
  Printf.printf
    "  reliable: fast_rtx=%d timeouts=%d retx=%d\n"
    rel_final.fast_retransmissions
    rel_final.timeouts
    rel_final.retransmissions;
  Printf.printf
    "  transport: fast_rtx=%d retx=%d\n"
    tr_final.fast_retransmissions
    tr_final.retransmissions
;;
