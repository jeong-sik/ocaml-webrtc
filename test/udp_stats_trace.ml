  ...
  for step = 1 to 40000 do
    if !collapse_step = 0 then begin
      ...
      if Sctp_full_transport.get_cwnd sender < 2000 &&
        ...
        Printf.printf "\n=== SCTP Stats ===\n";
        Printf.printf "Sender: msgs_sent=%d, retransmissions=%d\n" s_stats.messages_sent s_stats.retransmissions;
        Printf.printf "Receiver: msgs_recv=%d, sacks_sent=%d\n%!" r_stats.messages_recv r_stats.sacks_sent;

        Printf.printf "\n=== UDP Stats (before trace) ===\n";
        Printf.printf "Sender UDP: packets_sent=%d, send_errors=%d\n" s_udp.packets_sent s_udp.send_errors;
        Printf.printf "Receiver UDP: packets_recv=%d, recv_errors=%d\n%!" r_udp.packets_recv r_udp.recv_errors;
      ...
      if tick_num >= 1 && tick_num <= 10 then begin
        ...
        Printf.printf "Delta: %d packets sent but NOT received!\n"
