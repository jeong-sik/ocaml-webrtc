...
let () =
  ...
  test "split ordering matches RFC 5764" (fun () ->
    ...
    match
      Dtls_srtp.split_keying_material ~profile:Srtp.SRTP_AES128_CM_HMAC_SHA1_80 material
    with
    | Error e -> failwith e
    | Ok km ->
      ...
      assert_true "client_key starts at 0" (Bytes.get_uint8 km.client_key 0 = 0);
      assert_true
        "server_key starts at key_len"
        (Bytes.get_uint8 km.server_key 0 = params.cipher_key_len);
      assert_true
        "client_salt starts after keys"
        (Bytes.get_uint8 km.client_salt 0 = params.cipher_key_len * 2);
      assert_true
        "server_salt starts after client_salt"
  ...
  test "encode/decode use_srtp round-trip" (fun () ->
    let profiles =
      [ Srtp.SRTP_AES128_CM_HMAC_SHA1_80; Srtp.SRTP_AES128_CM_HMAC_SHA1_32 ]
    in
  ...
  test "negotiate_profile" (fun () ->
    ...
    match
      Dtls_srtp.negotiate_profile ~client_profiles:client ~server_supported:server
    with
    | None -> failwith "Expected negotiation success"
    | Some p ->
      assert_true
        "negotiated SHA1_80 (first client profile in server)"
  ...
  test "server_use_srtp single profile" (fun () ->
    ...
    match Dtls_srtp.decode_use_srtp_extension data with
    | Error e -> failwith e
    | Ok decoded -> assert_eq "single profile" 1 (List.length decoded.profiles));
