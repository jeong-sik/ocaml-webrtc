...
let () =
  ...
  for step = 31701 to 31850 do
    ...
    let send_result =
      if can then
        Sctp_full_transport.send_data sender ~stream_id:0 ~data
      else
        Error "skipped"
    in
    ...
    Printf.printf "step %d: can=%b result=%s | send_udp:%d recv_udp:%d tick_udp:%d | abs_sent=%d abs_recv=%d delta=%d flight=%d->%d\n%!"
      step can
      (match send_result with Ok n -> Printf.sprintf "Ok(%d)" n | Error e -> Printf.sprintf "Err(%s)" e)
      udp_by_send udp_recv udp_by_stick
      abs_sent_final abs_recv_final total_delta
      flight flight_after;

    if total_delta > 30 then begin
