...
let () =
  ...
  Printf.printf "At step 31770: sent=%d recv=%d\n\n%!"
    (Udp_transport.get_stats sender_udp).packets_sent
    (Udp_transport.get_stats receiver_udp).packets_recv;
  ...
  for step = 31771 to 31780 do
    ...
    Printf.printf "step %d: sent %d->%d->%d | recv %d->%d->%d | delta=%d flight=%d\n%!"
      step before after_send after_all
      recv_before recv_after recv_final
      (after_all - recv_final) flight
  done
