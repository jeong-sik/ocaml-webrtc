...
let config =
  { Replay_harness.src_port = 5000; dst_port = 5000;
    initial_tsn = 1000l; my_vtag = 0x12345678l;
    steps = [] }
...
let () =
  ...
  let trace = Replay_harness.record
    ...
    ~initial_tsn:1000l ~my_vtag:0x12345678l
    [] in
  check "empty trace records 0 steps"
    (List.length trace.steps = 0);
  check "empty trace replays OK"
    (Replay_harness.verify trace = Ok ());
  ()
...
let () =
  ...
  let t = Sctp_core.create
    ~src_port:5000 ~dst_port:5000
    ~initial_tsn:1000l ~my_vtag:0x12345678l () in
  Sctp_core.set_now t 0.0;
  let init_outputs = Sctp_core.initiate t in
  check "initiate produces outputs"
    (List.length init_outputs > 0);

  (* The initiate outputs contain a SendPacket — use that as replay input *)
  let init_packet = List.find_map (fun o ->
    match o with
    | Sctp_core.SendPacket p -> Some p
    | _ -> None
  ) init_outputs in
  check "initiate sends a packet"
    (Option.is_some init_packet);
  ()
...
let () =
  ...
  let trace = Replay_harness.record
    ...
    ~initial_tsn:1000l ~my_vtag:0x12345678l
    inputs in
  check "3 steps recorded"
    (List.length trace.steps = 3);
...
let () =
  ...
  let trace = Replay_harness.record
    ...
    ~initial_tsn:1000l ~my_vtag:0x12345678l
    inputs in
  check "4 steps recorded"
    (List.length trace.steps = 4);
  (match Replay_harness.verify trace with
...
let () =
  ...
  let trace = Replay_harness.record
    ~src_port:5000 ~dst_port:5000
    ~initial_tsn:42l ~my_vtag:0xDEADBEEFl
    inputs in
  check "8 steps recorded"
    (List.length trace.steps = 8);
  (match Replay_harness.verify trace with
...
let () =
  Printf.printf "\n=== Output Equality ===\n";
  check "SendPacket equal"
    (Replay_harness.output_equal
       (SendPacket (Bytes.of_string "abc"))
       (SendPacket (Bytes.of_string "abc")));
  check "SendPacket diff"
    (not (Replay_harness.output_equal
       (SendPacket (Bytes.of_string "abc"))
       (SendPacket (Bytes.of_string "xyz"))));
  check "Error equal"
    (Replay_harness.output_equal (Error "x") (Error "x"));
  check "Error diff"
    (not (Replay_harness.output_equal (Error "x") (Error "y")));
  check "cross-type diff"
    (not (Replay_harness.output_equal ConnectionEstablished ConnectionClosed));
  check "pp_divergence"
    (String.length (Replay_harness.pp_divergence 0 [] []) > 0);
  ignore config;
  ()
