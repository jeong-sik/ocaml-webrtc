...
let assert_eq32 msg a b =
  if a <> b then failwith (Printf.sprintf "%s: got %ld, expected %ld" msg a b)
;;
...
let test_rfc4960_sack () =
  ...
  test "SACK generation with gaps" (fun () ->
    ...
    let _ = Sctp_reliable.record_received reliable (Int32.add initial_tsn 1l) in
    (* TSN+1: OK *)
    let _ = Sctp_reliable.record_received reliable (Int32.add initial_tsn 3l) in
    (* TSN+3: gap! *)
    let _ = Sctp_reliable.record_received reliable (Int32.add initial_tsn 4l) in
    (* TSN+4: gap continues *)
