...
let assert_eq_state msg expected actual =
  if not (Ice_check.equal_check_state expected actual)
  then
    failwith
      (Printf.sprintf
         "%s: expected %s, got %s"
...
let test_state_transitions () =
  Printf.printf
    "\n\
     \xE2\x95\x90\xE2\x95\x90\xE2\x95\x90 State Transition Tests (RFC 8445 Section \
     6.1.2.6) \xE2\x95\x90\xE2\x95\x90\xE2\x95\x90\n";
  ...
  test "Frozen -> Waiting on Start_check" (fun () ->
    ...
    assert_eq_state
      "after first Start_check"
      Ice_check.Waiting
      (Ice_check.get_state check);
    match output with
    | Ice_check.No_op -> ()
  ...
  test "Waiting -> In_progress on Start_check (sends request)" (fun () ->
    ...
    assert_eq_state
      "after second Start_check"
      Ice_check.In_progress
      (Ice_check.get_state check);
    match output with
  ...
  test "In_progress -> Succeeded on valid response" (fun () ->
    ...
    let response =
      Ice_check.Stun_response_received
...
let test_retransmission () =
  Printf.printf
    "\n\
     \xE2\x95\x90\xE2\x95\x90\xE2\x95\x90 Retransmission Timer Tests (RFC 8445 Section \
     14.3) \xE2\x95\x90\xE2\x95\x90\xE2\x95\x90\n";
...
let test_transaction_matching () =
  Printf.printf
    "\n\
     \xE2\x95\x90\xE2\x95\x90\xE2\x95\x90 Transaction ID Matching Tests \
...
let test_nomination () =
  Printf.printf
    "\n\
     \xE2\x95\x90\xE2\x95\x90\xE2\x95\x90 Nomination Flag Tests (USE-CANDIDATE) \
     \xE2\x95\x90\xE2\x95\x90\xE2\x95\x90\n";
...
let test_check_list () =
  Printf.printf
    "\n\
     \xE2\x95\x90\xE2\x95\x90\xE2\x95\x90 Check List Operations Tests \
     \xE2\x95\x90\xE2\x95\x90\xE2\x95\x90\n";
  ...
  test "update_check modifies check in list" (fun () ->
    ...
    match Ice_check.get_check list id with
    | Some c -> assert_eq_state "updated state" Ice_check.Waiting (Ice_check.get_state c)
  ...
  test "get_next_waiting finds waiting checks" (fun () ->
    ...
    let check1 =
      match Ice_check.get_check list id1 with
      | Some c -> c
  ...
  test "get_succeeded returns succeeded checks" (fun () ->
    ...
    let check2 =
      match Ice_check.get_check list id2 with
      | Some c -> c
      | None -> failwith "not found"
    in
...
let test_edge_cases () =
  Printf.printf
    "\n\
     \xE2\x95\x90\xE2\x95\x90\xE2\x95\x90 Edge Cases Tests \
     \xE2\x95\x90\xE2\x95\x90\xE2\x95\x90\n";
  ...
  test "is_terminal for all states" (fun () ->
    ...
    let response =
      Ice_check.Stun_response_received
        { transaction_id = tx_id; success = true; mapped_addr = None; error_code = None }
    in
    let check, _ = Ice_check.step check response 2.0 in
...
let test_error_codes () =
  Printf.printf
    "\n\
     \xE2\x95\x90\xE2\x95\x90\xE2\x95\x90 Error Code Handling Tests \
     \xE2\x95\x90\xE2\x95\x90\xE2\x95\x90\n";
  let test_error_code code expected_msg =
    ...
    | Some msg ->
      assert_true
        (Printf.sprintf "error contains '%s'" expected_msg)
        (String.length msg > 0)
    | None -> failwith "Expected error message"
  in
  test "error 400 (Bad Request)" (fun () -> test_error_code 400 "Bad request");
  test "error 401 (Unauthorized)" (fun () -> test_error_code 401 "Unauthorized");
  test "error 487 (Role Conflict)" (fun () -> test_error_code 487 "Role conflict");
  test "unknown error code" (fun () -> test_error_code 999 "STUN error");
  test "error response with no code" (fun () ->
    ...
    let response =
      Ice_check.Stun_response_received
        { transaction_id = tx_id; success = false; mapped_addr = None; error_code = None }
    in
    let check, _ = Ice_check.step check response 2.0 in
    match Ice_check.get_error check with
...
let test_roles () =
  Printf.printf
    "\n\
     \xE2\x95\x90\xE2\x95\x90\xE2\x95\x90 ICE Role Tests (Controlling/Controlled) \
     \xE2\x95\x90\xE2\x95\x90\xE2\x95\x90\n";
...
let test_stun_attributes () =
  Printf.printf
    "\n\
     \xE2\x95\x90\xE2\x95\x90\xE2\x95\x90 STUN Request Attributes Tests \
     \xE2\x95\x90\xE2\x95\x90\xE2\x95\x90\n";
  test "username format is remoteUfrag:localUfrag" (fun () ->
    let check = create_test_check ~local_ufrag:"LOCAL" ~remote_ufrag:"REMOTE" () in
    let check, _ = Ice_check.step check Ice_check.Start_check 0.0 in
    let _, output = Ice_check.step check Ice_check.Start_check 1.0 in
    match output with
...
let () =
  Printf.printf
    "\n";
  Printf.printf
    "\xE2\x95\x91     ICE Connectivity Check Tests (RFC 8445)                          \
     \xE2\x95\x91\n";
  Printf.printf
    "\xE2\x95\x91     Sans-IO State Machine + Check List Operations                    \
     \xE2\x95\x91\n";
  Printf.printf
    "\n";
