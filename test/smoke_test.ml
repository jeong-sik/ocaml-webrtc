...
let () =
  ...
  | Ok alice, Ok bob ->
    ...
    | Ok shared_alice, Ok shared_bob ->
      ...
      if match_result then Printf.printf "   ✅ ECDHE P-256: PASS\n"
  ...
  let ciphertext = Webrtc_crypto.aes_gcm_encrypt ~key ~implicit_iv ~explicit_nonce ~aad ~plaintext in
  Printf.printf "   Plaintext: %d bytes\n" (Cstruct.length plaintext);
  Printf.printf "   Ciphertext: %d bytes (includes 16-byte tag)\n" (Cstruct.length ciphertext);

  (match Webrtc_crypto.aes_gcm_decrypt ~key ~implicit_iv ~explicit_nonce ~aad ~ciphertext_and_tag:ciphertext with
  | Ok decrypted ->
    let match_result = Cstruct.equal plaintext decrypted in
    Printf.printf "   Roundtrip match: %b\n" match_result;
    if match_result then Printf.printf "   ✅ AES-GCM: PASS\n"
