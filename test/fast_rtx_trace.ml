...
let () =
  ...
  Printf.printf "At 31700: sent=%d recv=%d delta=%d\n"
    s0.packets_sent r0.packets_recv (s0.packets_sent - r0.packets_recv);
  Printf.printf "         fast_rtx=%d timeouts=%d\n\n%!"
    rel0.fast_retransmissions rel0.timeouts;
  ...
  for step = 31701 to 31850 do
    ...
    let fast_delta = rel_post_tick.fast_retransmissions - rel_pre_tick.fast_retransmissions in
    ...
    if tick_pkts > 0 || timeout_delta > 0 || fast_delta > 0 || step mod 20 = 0 then begin
      Printf.printf "step %d: send:%d tick:%d | fast_rtx:%d timeout:%d retx:%d | delta=%d flight:%d->%d\n%!"
        step send_pkts tick_pkts fast_delta timeout_delta retx_delta delta flight_before flight_after
    end;

    if delta > 30 then begin
      Printf.printf "\n*** Delta > 30 at step %d ***\n%!" step;
      Printf.printf "Total: fast_rtx=%d timeouts=%d retx=%d\n"
        rel_post_tick.fast_retransmissions rel_post_tick.timeouts rel_post_tick.retransmissions;
      exit 0
    end
  done;
