...
let run_with_stub ~(responses : (bytes * string * int) list) ~(now : float) f =
    ...
    try_with
      ...
      { effc =
          (fun (type a) (eff : a Effect.t) ->
            ...
            | Turn.Now -> Some (fun (k : (a, _) continuation) -> continue k now)
            | Turn.Sleep _ -> Some (fun (k : (a, _) continuation) -> continue k ())
            | _ -> None)
      }
  in
...
let test_allocate_success () =
  ...
  test "allocate success sets active state" (fun () ->
    ...
    let attrs =
      bytes_concat
        [ build_xor_relayed_attr ~ip:"192.0.2.1" ~port:5000; build_lifetime_attr 600 ]
    in
    let responses = [ ok_response 0x0103 attrs ] in
    let result, _sends =
      run_with_stub ~responses ~now:1000.0 (fun () -> Turn.allocate t)
    in
    match result with
    | Result.Ok (ip, port) ->
  ...
  test "refresh lifetime=0 deallocates" (fun () ->
    ...
    let alloc_attrs =
      bytes_concat
        [ build_xor_relayed_attr ~ip:"192.0.2.2" ~port:5001; build_lifetime_attr 600 ]
    in
    let refresh_attrs = build_lifetime_attr 0 in
    let responses =
      [ ok_response 0x0103 alloc_attrs; ok_response 0x0104 refresh_attrs ]
    in
    let result, _sends =
      run_with_stub ~responses ~now:1000.0 (fun () ->
...
let test_allocate_auth_flow () =
  ...
  test "401 -> authenticated allocate" (fun () ->
    ...
    let ok_attrs =
      bytes_concat
        [ build_xor_relayed_attr ~ip:"192.0.2.3" ~port:5002; build_lifetime_attr 600 ]
    in
...
let test_permissions_and_channel_data () =
  ...
  test "CreatePermission + ChannelBind + channel data" (fun () ->
    ...
    let alloc_attrs =
      bytes_concat
        [ build_xor_relayed_attr ~ip:"192.0.2.4" ~port:5003; build_lifetime_attr 600 ]
    in
    let responses =
