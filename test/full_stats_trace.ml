...
let () =
  ...
  for step = 1 to 40000 do
    ...
    if step mod 5000 = 0 then begin
      ...
      Printf.printf "\n=== Step %d ===\n" step;
      Printf.printf "SCTP Sender: msgs_sent=%d sacks_recv=%d rtx=%d\n"
        s_sctp.messages_sent s_sctp.sacks_recv s_sctp.retransmissions;
      Printf.printf "SCTP Receiver: msgs_recv=%d sacks_sent=%d\n"
        r_sctp.messages_recv r_sctp.sacks_sent;
      Printf.printf "UDP Sender: pkts_sent=%d pkts_recv=%d\n"
        s_udp.packets_sent s_udp.packets_recv;
      Printf.printf "UDP Receiver: pkts_sent=%d pkts_recv=%d\n"
        r_udp.packets_sent r_udp.packets_recv;
      Printf.printf "Correlation: sender_data_pkts=%d vs receiver_udp_recv=%d (delta=%d)\n"
        s_udp.packets_sent r_udp.packets_recv (s_udp.packets_sent - r_udp.packets_recv);
      Printf.printf "Correlation: receiver_sack_udp=%d vs sender_udp_recv=%d (delta=%d)\n%!"
        r_udp.packets_sent s_udp.packets_recv (r_udp.packets_sent - s_udp.packets_recv)
    end;
    ...
    if Sctp_full_transport.get_cwnd sender < 2000 &&
      ...
      Printf.printf "\n*** COLLAPSE at step %d ***\n" step;
      Printf.printf "Flight: %d, cwnd: %d\n"
        (Sctp_full_transport.get_flight_size sender)
        (Sctp_full_transport.get_cwnd sender);
      Printf.printf "SCTP: sacks_sent=%d sacks_recv=%d DELTA=%d\n"
        r_sctp.sacks_sent s_sctp.sacks_recv (r_sctp.sacks_sent - s_sctp.sacks_recv);
      Printf.printf "UDP data: sent=%d recv=%d DELTA=%d\n"
        s_udp.packets_sent r_udp.packets_recv (s_udp.packets_sent - r_udp.packets_recv);
      Printf.printf "UDP SACK: sent=%d recv=%d DELTA=%d\n"
        r_udp.packets_sent s_udp.packets_recv (r_udp.packets_sent - s_udp.packets_recv);
      exit 0
    end
  done;
