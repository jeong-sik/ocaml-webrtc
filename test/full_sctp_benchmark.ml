...
let bench_congestion_control () =
  ...
  for i = 1 to 100 do
    ...
    if i mod 20 = 0 then
      Printf.printf "  After %d msgs: cwnd=%d flight=%d\n"
        i
        (Sctp_full_transport.get_cwnd pair.sender)
        (Sctp_full_transport.get_flight_size pair.sender)
  done;
...
let print_comparison ~single ~multi =
  Printf.printf "\n";
  Printf.printf "╔═══════════════════════════════════════════════════════════════════════╗\n";
  Printf.printf "║              FAIR COMPARISON (Full SCTP State Machine)               ║\n";
  Printf.printf "╠═══════════════════════════════════════════════════════════════════════╣\n";
  Printf.printf "║  Implementation       │ Single Conn │ 10 Conn  │ Threading │ Lang    ║\n";
  Printf.printf "╠═══════════════════════╪═════════════╪══════════╪═══════════╪═════════╣\n";
  Printf.printf "║  OCaml (Full SCTP)    │ %6.1f MB/s │ %6.1f MB/s │ Single    │ OCaml   ║\n"
    single multi;
  Printf.printf "║  Pion (Go)            │   ~150 MB/s │ 177.9 MB/s │ Goroutine │ Go      ║\n";
  Printf.printf "║  webrtc-rs (Rust)     │   ~100 MB/s │ 135.5 MB/s │ Async     │ Rust    ║\n";
  Printf.printf "║  RustRTC (Rust)       │   ~180 MB/s │ 213.4 MB/s │ Async     │ Rust    ║\n";
  Printf.printf "╚═══════════════════════╧═════════════╧══════════╧═══════════╧═════════╝\n";
  Printf.printf "\n";
