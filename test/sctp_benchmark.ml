...
let bench_batch_encoding () =
  ...
  let chunks = List.init 5 (fun i -> { base_chunk with tsn = Int32.of_int (1000 + i) }) in
  (* Individual encoding *)
  ...
  let chunks_10 =
    List.init 10 (fun i -> { base_chunk with tsn = Int32.of_int (1000 + i) })
  in
...
let bench_config () =
  ...
  Printf.printf
    "  - 256KB rwnd supports ~%d in-flight 1KB messages\n"
    (config.a_rwnd / 1024);
