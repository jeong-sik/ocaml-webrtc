...
let () =
  ...
  for _ = 1 to 35000 do
    ...
    if Sctp_full_transport.get_cwnd sender < 2000 &&
      ...
      Printf.printf "(RTO is 2.0s, so T3-rtx should fire after ~2 seconds worth of ticks)\n\n";
      ...
      for tick = 1 to 50 do
        ...
        if rtx > 0 || new_sacks > 0 || flight_after <> flight_before then
          Printf.printf "tick %d: rtx=%d sacks=%d flight=%d (delta=%d)\n"
            tick rtx new_sacks flight_after (flight_after - flight_before)
      done;
