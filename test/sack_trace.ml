...
let () =
  ...
  for i = 1 to 5 do
    ...
    Printf.printf "  Tick %d: msgs_recv=%d sacks_sent=%d\n" i stats.messages_recv stats.sacks_sent
  done;
  ...
  for i = 1 to 5 do
    ...
    Printf.printf "  Tick %d: sacks_recv=%d flight=%d cwnd=%d\n" 
      i stats.sacks_recv (Sctp_full_transport.get_flight_size sender) (Sctp_full_transport.get_cwnd sender)
  done;
  ...
  for i = 1 to 1000 do
    ...
    if i mod 200 = 0 then begin
      ...
      Printf.printf "  Iter %d: msgs=%d sacks_recv=%d ratio=%.2f sacks/iter\n"
        i !msgs sacks_this_period (float sacks_this_period /. float i)
    end
  done
