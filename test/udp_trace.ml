  ...
  for step = 1 to 40000 do
    if !collapse_step = 0 then begin
      ...
      if Sctp_full_transport.get_cwnd sender < 2000 &&
        ...
        Printf.printf "Retransmissions were sent. Waiting to see if packets arrive...\n\n%!";
        collapse_step := step;
        ...
        for tick = 1 to 5 do
          ...
          Printf.printf "  Receiver: msgs_recv delta=%d, sacks_sent delta=%d\n%!"
          ...
          Printf.printf "  Sender: sacks_recv delta=%d, retransmissions delta=%d\n%!"
        ...
        Printf.printf "the retransmitted packets are NOT being received by the receiver.\n";
