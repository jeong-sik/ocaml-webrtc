...
let () =
    ...
    (fun () ->
      while Unix.gettimeofday () < end_time do
        ...
        Printf.printf "[%.1fs] sent=%d recv=%d (%.1f%%) rtx=%d\n%!"
          ...
          (if sent > 0 then float_of_int receiver_stats.messages_recv /. float_of_int sent *. 100.0 else 0.0)
  ...
  if delivery_ratio < 99.0 then begin
    Printf.printf "\n  ⚠️  WARNING: Delivery ratio < 99%%!\n";
    Printf.printf "      Previous benchmark was MISLEADING!\n"
  end else if delivery_ratio >= 99.9 then begin
