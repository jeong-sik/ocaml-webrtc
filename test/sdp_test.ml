...
let sample_fingerprint value = { Sdp.hash_func = "sha-256"; fingerprint = value }
  ...
  test "create_answer updates session + setup" (fun () ->
    ...
    let answer =
      Sdp.create_answer
        ...
        ~ice_ufrag:"o2"
        ~ice_pwd:"p2"
        ~fingerprint:(sample_fingerprint "33:44")
    in
    assert_eq_i64
      "sess_version"
      (Int64.succ offer.origin.sess_version)
      answer.origin.sess_version;
    ...
    | m :: _ -> assert_true "setup active" (m.setup = Some "active"));
