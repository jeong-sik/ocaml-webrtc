...
let print_comparison ~single ~multi =
  ...
  Printf.printf "║    Single Connection:  %6.1f MB/s                            ║\n" single;
...
let () =
  ...
  let single_throughput = bench_single_connection ~packet_size:1024 ~duration_sec:duration in
  let multi_throughput = bench_multi_connection ~num_connections:10 ~packet_size:1024 ~duration_sec:duration in
