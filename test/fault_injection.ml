...
let truncate_packet ~len data =
  ...
  if len >= actual then data else Bytes.sub data 0 len
;;
...
let fire_all_timers t =
  ...
  List.concat_map (fun timer -> Sctp_core.handle t (Sctp_core.TimerFired timer)) timers
;;
