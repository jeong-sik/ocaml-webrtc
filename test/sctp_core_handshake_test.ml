...
let receive_packet t packet =
  Sctp_core.handle t (Sctp_core.PacketReceived packet)
...
let test_initiate () =
  ...
  test "initiate_direct() goes directly to Established" (fun () ->
    ...
    assert_state "after initiate_direct" Sctp_core.Established (Sctp_core.get_conn_state client)
  )
...
let test_full_handshake () =
  ...
  test "complete handshake flow" (fun () ->
    ...
    assert_state "client after INIT" Sctp_core.CookieWait (Sctp_core.get_conn_state client);
    ...
    assert_state "client after INIT-ACK" Sctp_core.CookieEchoed (Sctp_core.get_conn_state client);
    let cookie_echo_packet = List.hd cookie_echo_packets in
    Printf.printf "    COOKIE-ECHO packet size: %d bytes\n%!" (Bytes.length cookie_echo_packet);
    ...
    assert_state "server after COOKIE-ECHO" Sctp_core.Established (Sctp_core.get_conn_state server);
    let cookie_ack_packet = List.hd cookie_ack_packets in
    Printf.printf "    COOKIE-ACK packet size: %d bytes\n%!" (Bytes.length cookie_ack_packet);
    ...
    assert_state "client after COOKIE-ACK" Sctp_core.Established (Sctp_core.get_conn_state client);
...
let test_edge_cases () =
  ...
  test "receiving INIT when not in Closed state" (fun () ->
    ...
    assert_state "client2 in CookieWait" Sctp_core.CookieWait (Sctp_core.get_conn_state client2);
  ...
  test "double initiate is no-op" (fun () ->
    ...
    assert_state "after second initiate" Sctp_core.CookieWait (Sctp_core.get_conn_state client);
    Printf.printf "    (First: %d outputs, Second: %d outputs)\n%!"
