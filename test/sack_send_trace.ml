...
let () =
  ...
  for step = 1 to 40000 do
    ...
    if s_udp.send_errors <> !last_send_err || r_udp.send_errors <> !last_recv_err
    then (
      Printf.printf
        "step %d: sender_send_err=%d receiver_send_err=%d (SACK fails!)\n%!"
        step
        s_udp.send_errors
        r_udp.send_errors;
    ...
    then (
      Printf.printf "\n*** COLLAPSE at step %d ***\n" step;
      Printf.printf
        "Sender UDP: sent=%d send_errors=%d recv_errors=%d\n"
        s_udp.packets_sent
        s_udp.send_errors
        s_udp.recv_errors;
      Printf.printf
        "Receiver UDP: sent=%d send_errors=%d recv_errors=%d\n"
        r_udp.packets_sent
        r_udp.send_errors
        r_udp.recv_errors;
    ...
    then
      Printf.printf
        "[checkpoint step %d: msgs=%d sender_err=%d receiver_err=%d]\n%!"
        step
        !msgs
        s_udp.send_errors
        r_udp.send_errors
