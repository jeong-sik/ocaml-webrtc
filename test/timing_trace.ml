...
let () =
  ...
  Printf.printf "Warmup took %.3f seconds (%.3f ms per step)\n\n%!"
  ...
  for step = 31701 to 31850 do
    ...
    if tick_udp > 0 || timeouts > 0 || step mod 20 = 0 || flight_before = 0 then begin
      Printf.printf "step %d: %.3fms | tick_udp:%d timeout:%d | delta=%d flight:%d->%d rto:%.3f\n%!"
        step (step_duration *. 1000.0) tick_udp timeouts delta flight_before flight_after rto
    end;
