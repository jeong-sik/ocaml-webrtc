...
let[@warning "-32"] assert_eq msg expected actual =
  if expected <> actual then
    failwith (Printf.sprintf "%s: expected %s, got different" msg expected)
...
let test_hmac_cookies () =
  ...
  test "verify_cookie rejects wrong client_addr" (fun () ->
    with_effects (fun () ->
      ...
      let client_addr2 = ("192.168.1.101", 12345) in  (* Different IP *)
  ...
  test "verify_cookie rejects wrong port" (fun () ->
    with_effects (fun () ->
      let client_addr1 = ("192.168.1.100", 12345) in
      let client_addr2 = ("192.168.1.100", 54321) in  (* Different port *)
  ...
  test "verify_cookie rejects wrong client_random" (fun () ->
    with_effects (fun () ->
      ...
      let client_random2 = Bytes.make 32 '\xFF' in  (* Different random *)
  ...
  test "check_retransmit_needed returns false when inactive" (fun () ->
    ...
    try_with
      ...
      { effc = fun (type a) (eff : a Effect.t) ->
          match eff with
          | Dtls.Now -> Some (fun (k : (a, _) continuation) ->
