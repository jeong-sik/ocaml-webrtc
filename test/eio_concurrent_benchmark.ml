  ...
  Eio.Fiber.all [
    ...
    (fun () ->
      ...
      while Unix.gettimeofday () < end_time do
        ...
        Printf.printf "  [%.1fs] msgs=%d (+%d/s) throughput=%.1f MB/s cwnd=%d\n%!"
          elapsed msgs delta throughput (Eio_sctp_full_transport.get_cwnd sender)
      done
    );
  ];
...
let bench_multi_connection ~num_connections =
  ...
  let fibers = List.concat_map (fun (sender, receiver) ->
    [
      (* Sender *)
      (fun () ->
        while Unix.gettimeofday () < end_time do
...
let () =
  Printf.printf "╔═══════════════════════════════════════════════════════════════════════╗\n";
  Printf.printf "║              Eio Concurrent SCTP Benchmark Suite                      ║\n";
  Printf.printf "╚═══════════════════════════════════════════════════════════════════════╝\n";
  ...
  Printf.printf "╠═══════════════════════════════════════════════════════════════════════╣\n";
  Printf.printf "║  Implementation       │ Single Conn │ 10 Conn  │ Model       │ Lang  ║\n";
  Printf.printf "╠═══════════════════════╪═════════════╪══════════╪═════════════╪═══════╣\n";
  Printf.printf "║  OCaml (Eio Fibers)   │  %6.1f MB/s │ %6.1f MB/s │ Concurrent  │ OCaml ║\n" single_tp multi_tp;
  Printf.printf "║  OCaml (Blocking)     │   117.1 MB/s │  115.7 MB/s │ Single-thrd │ OCaml ║\n";
  Printf.printf "║  Pion (Go)            │  ~150.0 MB/s │  177.9 MB/s │ Goroutines  │ Go    ║\n";
  Printf.printf "║  RustRTC (Rust)       │  ~180.0 MB/s │  213.4 MB/s │ Async       │ Rust  ║\n";
  Printf.printf "╚═══════════════════════╧═════════════╧══════════╧═════════════╧═══════╝\n"
