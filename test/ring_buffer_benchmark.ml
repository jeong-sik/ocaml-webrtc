...
let bench_hashtbl () =
  ...
  Printf.printf
    "  Enqueue:     %.3f s (%.0f ops/s)\n"
    enqueue_time
    (float_of_int iterations /. enqueue_time);
  Printf.printf
    "  Dequeue:     %.3f s (%.0f ops/s)\n"
    dequeue_time
    (float_of_int iterations /. dequeue_time);
...
let bench_ring_buffer () =
  ...
  for _ = 0 to iterations - 1 do
    let chunk =
      ...
      ; tsn = 0l
      ; (* Will be assigned by ring buffer *)
        stream_id = 0
      ; stream_seq = 0
      ; ppid = 0l
      ; user_data = data
...
let bench_buffer_pool () =
  ...
  let pool =
    Buffer_pool.create ~config:{ Buffer_pool.buffer_size = 1024; pool_size = 1024 } ()
  in
  let start = Unix.gettimeofday () in
  for _ = 0 to iterations - 1 do
  ...
  Printf.printf "  Iterations:    %d\n" iterations;
  Printf.printf
    "  Bytes.create:  %.3f s (%.0f ops/s)\n"
    heap_time
    (float_of_int iterations /. heap_time);
  Printf.printf
    "  Buffer Pool:   %.3f s (%.0f ops/s)\n"
    pool_time
    (float_of_int iterations /. pool_time);
...
let () =
  ...
  Printf.printf
    "║  Ring Buffer      │ %9.0f   │ %.1fx                        ║\n"
    ring_ops
    (ring_ops /. htbl_ops);
  Printf.printf
    "║  Buffer Pool      │      -      │ %.1fx vs heap alloc         ║\n"
    pool_speedup;
  Printf.printf "╚═══════════════════╧═════════════╧═════════════════════════════╝\n";
  Printf.printf "\n";
  Printf.printf "Key findings:\n";
  Printf.printf
    "  • Ring buffer: %.1fx faster than Hashtbl for SCTP queue ops\n"
    (ring_ops /. htbl_ops);
  Printf.printf "  • Buffer pool: %.1fx faster than heap allocation\n" pool_speedup;
  Printf.printf
    "  • Combined potential improvement: ~%.0fx\n"
