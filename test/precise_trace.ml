...
let () =
  ...
  for step = 31701 to 32000 do
    ...
    (* send_data *)
    let _sent_ok =
      if flight_start < Sctp_full_transport.get_cwnd sender
      then (
    ...
    let _sacks_sent_by_recv =
      s_after_recv_tick.packets_recv - s_after_send.packets_recv
    in
    (* sender tick *)
    Sctp_full_transport.tick sender;
    ...
    if delta <> !last_delta || step mod 10 = 0
    then (
      Printf.printf
        "step %d: d=%d->%d | send_data:%d sent, recv_tick:%d recv, sender_tick:%d sent | \
         flight:%d->%d\n\
         %!"
        step
        !last_delta
