...
let assert_equal msg a b = if a <> b then failwith (Printf.sprintf "%s: %s <> %s" msg (string_of_int a) (string_of_int b))
...
let test_ecdhe () =
  ...
  test "Key exchange" (fun () ->
    ...
    | Ok alice, Ok bob ->
      ...
      (match Ecdhe.compute_shared_secret ~keypair:alice ~peer_public_key:bob_pub,
             Ecdhe.compute_shared_secret ~keypair:bob ~peer_public_key:alice_pub with
      | Ok s1, Ok s2 ->
  ...
  test "Multiple exchanges produce different secrets" (fun () ->
    ...
    | Ok a, Ok b, Ok c ->
      let ab = Ecdhe.compute_shared_secret ~keypair:a ~peer_public_key:(Ecdhe.public_key b) in
      let ac = Ecdhe.compute_shared_secret ~keypair:a ~peer_public_key:(Ecdhe.public_key c) in
      (match ab, ac with
      | Ok s1, Ok s2 -> assert_true "Different peers should produce different secrets" (not (Cstruct.equal s1 s2))
...
let test_aes_gcm () =
  ...
  test "Encrypt/decrypt roundtrip" (fun () ->
    ...
    for i = 0 to 15 do Cstruct.set_uint8 key i (i + 1) done;
    ...
    let ciphertext = Webrtc_crypto.aes_gcm_encrypt ~key ~implicit_iv:iv ~explicit_nonce:nonce ~aad ~plaintext in
    assert_true "Ciphertext should be plaintext + 16 bytes tag"
      (Cstruct.length ciphertext = Cstruct.length plaintext + 16);

    match Webrtc_crypto.aes_gcm_decrypt ~key ~implicit_iv:iv ~explicit_nonce:nonce ~aad ~ciphertext_and_tag:ciphertext with
    | Ok decrypted -> assert_true "Decryption should match" (Cstruct.equal plaintext decrypted)
    | Error e -> failwith e
  );
  ...
  test "Different AAD fails" (fun () ->
    ...
    let ciphertext = Webrtc_crypto.aes_gcm_encrypt ~key ~implicit_iv:iv ~explicit_nonce:nonce ~aad:aad1 ~plaintext in

    match Webrtc_crypto.aes_gcm_decrypt ~key ~implicit_iv:iv ~explicit_nonce:nonce ~aad:aad2 ~ciphertext_and_tag:ciphertext with
...
let test_tls_prf () =
  ...
  test "PRF produces deterministic output" (fun () ->
    ...
    let seed = Cstruct.of_string "client_random_32_bytes__________server_random_32_bytes__________" in
  ...
  test "Different labels produce different output" (fun () ->
    ...
    assert_true "Different labels should produce different output" (not (Cstruct.equal out1 out2))
  );
  ...
  test "Key material expansion" (fun () ->
    ...
    let master_secret = Webrtc_crypto.derive_master_secret
      ~pre_master_secret:pre_master ~client_random ~server_random in

    (* Then derive key material from master secret *)
    let keys = Webrtc_crypto.derive_key_material ~master_secret
      ~client_random ~server_random ~key_size:16 ~iv_size:4 in

    assert_true "Client write key should be 16 bytes" (Cstruct.length keys.client_write_key = 16);
    assert_true "Server write key should be 16 bytes" (Cstruct.length keys.server_write_key = 16);
    assert_true "Client write IV should be 4 bytes" (Cstruct.length keys.client_write_iv = 4);
    assert_true "Server write IV should be 4 bytes" (Cstruct.length keys.server_write_iv = 4)
  )
...
let test_sctp () =
  ...
  test "Data fragmentation" (fun () ->
    let large_data = Bytes.make 5000 'X' in  (* Larger than MTU *)
    let fragments = Sctp.fragment_data
      ~data:large_data ~stream_id:1 ~stream_seq:0 ~ppid:51l ~start_tsn:1000l ~mtu:1280 in
  ...
  test "Batch encoding" (fun () ->
    ...
    assert_true "Small chunks should be batched into fewer packets" (List.length packets <= 2)
  )
...
let test_ice () =
  ...
  test "Remote candidate adds and forms pairs" (fun () ->
    ...
    let local : Ice.candidate = {
      ...
      port = 5000;
      cand_type = Ice.Host;
      base_address = None; base_port = None; related_address = None; related_port = None;
      extensions = [];
    } in
