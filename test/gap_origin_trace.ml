...
let () =
  ...
  for step = 1 to 5000 do
    ...
    if gap_count_after_rtick > 0 && !first_gap_step = None
    then (
    ...
    if step mod 500 = 0 || gap_count_after <> gap_count_before
    then
      Printf.printf
        "step %d: gaps=%d cum_tsn=%ld sent=%b tick_pkts=%d\n%!"
        step
        gap_count_after
        cum_tsn_after_rtick
        sent
    ...
    if !first_gap_step <> None && step > Option.get !first_gap_step + 50
    then (
