...
let () =
  ...
  for step = 1 to 50000 do
    ...
    if delta <> !last_delta then begin
      Printf.printf "step %d: delta %d->%d | sent_this=%d recv_this=%d | cwnd=%d flight=%d | sent=%b\n%!"
        step !last_delta delta packets_sent_this_step packets_recv_this_step cwnd_before flight_before sent_this_step;
      last_delta := delta
    end;
    ...
    if Sctp_full_transport.get_cwnd sender < 2000 &&
      ...
      Printf.printf "\n*** COLLAPSE at step %d ***\n" step;
      Printf.printf "Total sent: %d, recv: %d, delta: %d\n"
        s_final.packets_sent r_final.packets_recv delta;
      exit 0
    end
  done;
