...
let () =
  ...
  Printf.printf
    "  sender: sacks_recv=%d sacks_sent=%d\n"
    s_stats.sacks_recv
    s_stats.sacks_sent;
  Printf.printf
    "  receiver: sacks_recv=%d sacks_sent=%d\n"
    r_stats.sacks_recv
    r_stats.sacks_sent;
  Printf.printf
    "  flight_size=%d cwnd=%d\n\n%!"
  ...
  for step = 31701 to 31850 do
    ...
    if step mod 10 = 0 || flight_change > 1040 || sack_delta > 0
    then
      Printf.printf
        "step %d: recv_data:%d sack_sent_by_recv:%d sack_recv_by_sender:%d | \
         flight:%d->%d (change:%d)\n\
         %!"
        step
        recv_delta
