...
let () =
  ...
  Printf.printf
    "At step 31700: sent=%d recv=%d delta=%d\n"
    s0.packets_sent
    r0.packets_recv
    (s0.packets_sent - r0.packets_recv);
  Printf.printf
    "RTO: %.3fs  cwnd: %d  flight: %d\n\n%!"
  ...
  for step = 31701 to 32000 do
    ...
    let t3_rtx_delta =
      rel_stats_after.retransmissions - rel_stats_pre_tick.retransmissions
    in
    let fast_rtx_delta =
      rel_stats_after.fast_retransmissions - rel_stats_pre_tick.fast_retransmissions
    in
    ...
    (* Report on changes *)
    if delta <> !last_delta || step mod 20 = 0 || timeouts > 0 || pkts_by_sender_tick > 0
    then (
      Printf.printf
        "step %d: delta=%d->%d | send:%b->%d recv:%d | tick_sent:%d (t3:%d fast:%d \
         timeout:%d) | flight:%d->%d cwnd:%d rto:%.3f->%.3f\n\
         %!"
        step
        !last_delta
        delta
