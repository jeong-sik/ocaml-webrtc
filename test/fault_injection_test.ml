...
let fresh_core () =
  Sctp_core.create ~src_port:5000 ~dst_port:5000 ()
...
let () =
  ...
  (* Too-short header *)
  let t = fresh_core () in
  let outputs = Sctp_core.handle t
    (PacketReceived (Fault_injection.too_short_header ())) in
  check "3-byte header returns Error"
    (Fault_injection.has_error outputs);
  check "3-byte header no crash"
    true;

  (* Header-only (no chunks) *)
  let t = fresh_core () in
  let header = Fault_injection.valid_header () in
  check "12-byte header-only no crash"
    (Fault_injection.did_not_crash (fun () ->
       Sctp_core.handle t (PacketReceived header)));
  ...
  for len = 1 to 20 do
    ...
    check (Printf.sprintf "truncated %d bytes no crash" len)
      (Fault_injection.did_not_crash (fun () ->
...
let () =
  ...
  for pos = 0 to 11 do
    for bit = 0 to 7 do
      ...
      check (Printf.sprintf "flip byte %d bit %d no crash" pos bit)
        (Fault_injection.did_not_crash (fun () ->
...
let () =
  ...
  check "zeroed CRC no crash"
    (Fault_injection.did_not_crash (fun () ->
       Sctp_core.handle t (PacketReceived zeroed)));

  (* Corrupt CRC specifically *)
  let t = fresh_core () in
  let bad_crc = Fault_injection.corrupt_byte ~pos:8 ~value:0xFF header in
  check "bad CRC byte no crash"
    (Fault_injection.did_not_crash (fun () ->
       Sctp_core.handle t (PacketReceived bad_crc)));
...
let () =
  ...
  let t = fresh_core () in
  check "timer after UserClose no crash"
    (Fault_injection.did_not_crash (fun () ->
...
let () =
  ...
  List.iter (fun msg ->
    ...
    check (Printf.sprintf "classify '%s' = %s" msg
