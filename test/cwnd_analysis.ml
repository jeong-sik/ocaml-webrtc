...
let () =
  ...
  Printf.printf "  packets that fit in cwnd: %d\n"
  ...
  while Sctp_full_transport.get_flight_size sender < Sctp_full_transport.get_cwnd sender do
  ...
  Printf.printf "  Instantaneous throughput: %.2f MB/s\n"
  ...
  Printf.printf "  flight_size after ticks: %d bytes\n" (Sctp_full_transport.get_flight_size sender);
  Printf.printf "  can_send after ticks: %b\n"
    (Sctp_full_transport.get_flight_size sender < Sctp_full_transport.get_cwnd sender);
  Printf.printf "\n";
  ...
  Printf.printf "  ns per tick: %.0f\n" (elapsed /. float_of_int iterations *. 1_000_000_000.0);
  Printf.printf "\n";

  (* Key insight *)
  Printf.printf "╔═══════════════════════════════════════════════════════════════╗\n";
  Printf.printf "║  ANALYSIS:                                                    ║\n";
  Printf.printf "║  - Initial cwnd = %d bytes (%d packets)               ║\n"
    (Sctp_full_transport.get_cwnd sender)
    (Sctp_full_transport.get_cwnd sender / packet_size);
  Printf.printf "║  - We can only send %d packets before blocking!          ║\n"
    !sent_count;
