...
let () =
  ...
  Printf.printf "\n=== VERIFICATION ===\n";
  if receiver_stats.messages_recv > 0
  then
    Printf.printf
      "✓ Receiver IS receiving data! (%d messages)\n"
      receiver_stats.messages_recv
  else Printf.printf "✗ WARNING: Receiver got 0 messages!\n";
  let ratio =
    if sender_stats.messages_sent > 0
    then
      float_of_int receiver_stats.messages_recv
      /. float_of_int sender_stats.messages_sent
      *. 100.0
    else 0.0
  in
