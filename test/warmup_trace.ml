...
let () =
  ...
  for step = 1 to 40000 do
    ...
    if step mod 5000 = 0 && step <> !last_report then begin
      last_report := step;
      ...
      Printf.printf "step %5d: msgs=%d sent=%d recv=%d DELTA=%d cwnd=%d flight=%d rtx=%d\n%!"
        step !msgs s_udp.packets_sent r_udp.packets_recv delta cwnd flight s.retransmissions;
