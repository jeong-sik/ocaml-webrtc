...
let () =
  ...
  let receiver =
    Sctp_eio.create ~initial_tsn ~host:"127.0.0.1" ~port:(base_port + 1) ()
  in
  ...
  while Unix.gettimeofday () -. start_time < float_of_int test_duration_sec do
    (* Try to send *)
    (match Sctp_eio.send sender ~stream_id:0 ~data with
     | Ok n ->
       incr send_count;
  ...
  let receiver_udp_stats =
    Udp_transport.get_stats (Sctp_eio.get_udp_transport receiver)
  in
  (* Print results *)
  Printf.printf "\n═══ SANS-IO RESULTS ═══\n\n";
  ...
  Printf.printf "\n  ═══ DELIVERY METRICS ═══\n";
  let delivery_ratio =
    if sender_stats.messages_sent > 0
    then 100.0 *. float_of_int !recv_count /. float_of_int sender_stats.messages_sent
    else 0.0
  in
  ...
  Printf.printf "\n  ═══ THROUGHPUT (HONEST) ═══\n";
  let send_throughput =
    float_of_int sender_stats.bytes_sent /. elapsed /. 1024.0 /. 1024.0
  in
