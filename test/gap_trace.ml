...
let () =
  ...
  Printf.printf
    "  UDP: sender_sent=%d receiver_recv=%d delta=%d\n"
    s_udp.packets_sent
    r_udp.packets_recv
  ...
  for step = 31701 to 31850 do
    ...
    if tick_pkts > 0 || total_delta > 10 || step mod 20 = 0 || send_errors > 0
    then
      Printf.printf
        "step %d: send:%d tick:%d | recv_rtick:%d | delta=%d flight:%d->%d errors=%d\n%!"
        step
        send_pkts
        tick_pkts
        recv_by_rtick
