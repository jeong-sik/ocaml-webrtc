...
let () =
  ...
  let _ = Lifecycle.advance lc ~phase:Lifecycle.Ice_gathering
    ~cleanup:(make_cleanup "ice") in
  let _ = Lifecycle.advance lc ~phase:Lifecycle.Dtls_handshake
    ~cleanup:(make_cleanup "dtls") in
  let _ = Lifecycle.advance lc ~phase:Lifecycle.Sctp_association
    ~cleanup:(make_cleanup "sctp") in

  check "lifecycle at Sctp_association"
  ...
  List.iter (fun msg ->
    ...
    check (Printf.sprintf "error '%s' classified" msg)
  ...
  check "failure classified as Protocol"
    (failure.error.cls = Protocol);
  check "3 cleanups ran"
    (List.length !cleanup_log = 3);
  check "cleanup order: sctp, dtls, ice"
...
let () =
  ...
  let trace = Replay_harness.record
    ~src_port:5000 ~dst_port:5000
    ~initial_tsn:999l ~my_vtag:0xCAFEBABEl
    inputs in

  check "6 steps recorded"
    (List.length trace.steps = 6);

  (* Verify determinism *)
  (match Replay_harness.verify trace with
   | Ok () -> check "fault injection replay deterministic" true
   | Error (idx, _, _) ->
     check (Printf.sprintf "replay failed at step %d" idx) false);

  (* Verify all errors are properly classified *)
  let all_errors = List.concat_map (fun step ->
    Fault_injection.errors_of step.Replay_harness.outputs
  ) trace.steps in
  check "some errors produced"
    (List.length all_errors > 0);
  List.iter (fun msg ->
    let err = Oas_error.of_string msg in
    check (Printf.sprintf "replay error classified: %s"
...
let () =
  ...
  List.iter (fun (error_msg, expected_cls) ->
    cleanup_log := [];
    let lc = Lifecycle.create () in
    let _ = Lifecycle.advance lc ~phase:Lifecycle.Ice_gathering
      ~cleanup:(make_cleanup "ice") in
    let failure = Lifecycle.fail lc ~error:error_msg in
    check (Printf.sprintf "'%s' → %s"
             error_msg (Oas_error.show_error_class expected_cls))
      (failure.error.cls = expected_cls);
    check (Printf.sprintf "'%s' cleanup ran" error_msg)
      (List.length !cleanup_log = 1)
  ) test_cases;
  ()
...
let () =
  ...
  check "cleanup order newest first"
