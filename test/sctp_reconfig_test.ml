...
let build_reconfig_packet params =
  ...
  Sctp.encode_packet
    { Sctp.header =
        { source_port = 5000; dest_port = 5000; verification_tag = 0l; checksum = 0l }
    ; chunks = [ chunk ]
    }
;;
...
let () =
  ...
  test "encode/decode outgoing reset" (fun () ->
    let params =
      [ Sctp_reconfig.Outgoing_ssn_reset
          { request_seq = 1l; response_seq = 2l; last_tsn = 100l; streams = [ 0; 1; 2 ] }
      ]
    in
  ...
  test "encode/decode response" (fun () ->
    let params = [ Sctp_reconfig.Reconfig_response { response_seq = 7l; result = 0l } ] in
    let encoded = Sctp_reconfig.encode_params params in
    match Sctp_reconfig.decode_params encoded with
    | Error e -> failwith e
  ...
  test "peer outgoing reset does not reset local outgoing seq" (fun () ->
    ...
    let out1 =
      Sctp_core.handle
        core
        (Sctp_core.UserSend { stream_id = 1; data = Bytes.of_string "a" })
    in
    let out2 =
      Sctp_core.handle
        core
        (Sctp_core.UserSend { stream_id = 1; data = Bytes.of_string "b" })
    in
    let seq1 =
      match find_send_packet out1 with
      | None -> failwith "missing packet"
      | Some p -> decode_data_stream_seq p
    in
    let seq2 =
      match find_send_packet out2 with
      | None -> failwith "missing packet"
      | Some p -> decode_data_stream_seq p
    in
    ...
    let reconfig =
      Sctp_reconfig.Outgoing_ssn_reset
        { request_seq = 10l; response_seq = 0l; last_tsn = 0l; streams = [ 1 ] }
    in
    ...
    let out3 =
      Sctp_core.handle
        core
        (Sctp_core.UserSend { stream_id = 1; data = Bytes.of_string "c" })
    in
    let seq3 =
      match find_send_packet out3 with
      | None -> failwith "missing packet"
      | Some p -> decode_data_stream_seq p
    in
  ...
  test "UserResetStreams resets after response" (fun () ->
    ...
    ignore
      (Sctp_core.handle
         ...
         (Sctp_core.UserSend { stream_id = 1; data = Bytes.of_string "b" }));
    let reset_out =
      Sctp_core.handle core (Sctp_core.UserResetStreams { stream_ids = [ 1 ] })
    in
    let reset_packet =
      match find_send_packet reset_out with
      | None -> failwith "missing reset packet"
      | Some p -> p
    in
    let request_seq = decode_reconfig_params reset_packet |> find_reconfig_request_seq in
    let out3 =
      Sctp_core.handle
        core
        (Sctp_core.UserSend { stream_id = 1; data = Bytes.of_string "c" })
    ...
    let response =
      Sctp_reconfig.Reconfig_response
        { response_seq = request_seq; result = Sctp_reconfig.result_success }
    in
    ...
    let out4 =
      Sctp_core.handle
        core
        (Sctp_core.UserSend { stream_id = 1; data = Bytes.of_string "d" })
    in
    let seq4 =
      match find_send_packet out4 with
      | None -> failwith "missing packet"
      | Some p -> decode_data_stream_seq p
    in
  ...
  test "incoming reset waits for last_tsn" (fun () ->
    ...
    let reconfig =
      Sctp_reconfig.Outgoing_ssn_reset
        { request_seq = 11l; response_seq = 0l; last_tsn = 10l; streams = [ 1 ] }
    in
    let packet = build_reconfig_packet [ reconfig ] in
    let outputs = Sctp_core.handle core (Sctp_core.PacketReceived packet) in
    let response_packet =
      match find_send_packet outputs with
