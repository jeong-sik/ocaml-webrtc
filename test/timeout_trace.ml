...
let () =
  ...
  for i = 1 to 3000 do
    ...
    if cwnd < !prev_cwnd / 2 || stats.retransmissions > stats_before.retransmissions
    then (
      ...
      Printf.printf "  rto: %.3f s\n" (Sctp_full_transport.get_rto sender);
      Printf.printf
        "  sacks_recv: %d (delta: %d)\n"
        stats.sacks_recv
        (stats.sacks_recv - stats_before.sacks_recv);
