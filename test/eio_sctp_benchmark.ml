  ...
  Eio.Fiber.all [
    ...
    (fun () ->
      while Atomic.get running do
        if Sctp_full_transport.get_flight_size pair.sender <
           Sctp_full_transport.get_cwnd pair.sender then begin
...
let bench_multicore ~num_domains =
  ...
  Eio.Fiber.all (
    List.init num_domains (fun i ->
      fun () -> Eio.Domain_manager.run dm (worker i)
    )
  );
...
let () =
  ...
  Printf.printf "\n";
  Printf.printf "╔════════════════════════════════════════════════════════════════════════╗\n";
  Printf.printf "║                    CONCURRENCY COMPARISON                              ║\n";
  Printf.printf "╠════════════════════════════════════════════════════════════════════════╣\n";
  Printf.printf "║  Mode                 │ Throughput │ Speedup │ Notes                   ║\n";
  Printf.printf "╠═══════════════════════╪════════════╪═════════╪═════════════════════════╣\n";
  Printf.printf "║  Single-threaded      │ %6.1f MB/s │   1.0x  │ Baseline                ║\n" single;
  Printf.printf "║  Eio Fibers (1 core)  │ %6.1f MB/s │  %4.1fx  │ Cooperative scheduling  ║\n"
    concurrent (concurrent /. single);
  Printf.printf "║  Eio Domains (%d cores)│ %6.1f MB/s │  %4.1fx  │ True parallelism        ║\n"
    recommended_cores multicore (multicore /. single);
  Printf.printf "║  Pion (Go goroutines) │ ~150.0 MB/s │  %4.0fx  │ M:N threading           ║\n"
    (150.0 /. single);
  Printf.printf "╚═══════════════════════╧════════════╧═════════╧═════════════════════════╝\n";
  Printf.printf "\n";
  Printf.printf "Analysis:\n";
  Printf.printf "  • Eio Fibers: No speedup (cooperative scheduling, single core)\n";
  Printf.printf "  • Eio Domains: %.1fx speedup (true parallelism across %d cores)\n"
    (multicore /. single) recommended_cores;
  Printf.printf "  • Pion gap: %.1fx difference (Go runtime + optimized data structures)\n"
    (150.0 /. multicore);
  Printf.printf "\n";
