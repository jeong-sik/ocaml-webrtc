...
let () =
  Printf.printf "╔═══════════════════════════════════════════════════════════════╗\n";
  Printf.printf "║     Full SCTP Path Profiling (%d iterations)             ║\n" iterations;
  Printf.printf "╚═══════════════════════════════════════════════════════════════╝\n\n";
  ...
  bench "Sctp.encode_data_chunk" (fun () ->
    Sctp.encode_data_chunk chunk
  );

  let encoded = Sctp.encode_data_chunk chunk in
  bench "Sctp.decode_data_chunk" (fun () ->
    Sctp.decode_data_chunk encoded
  );

  (* Fragment data *)
  bench "Sctp.fragment_data (1KB → 1 chunk)" (fun () ->
    Sctp.fragment_data ~data ~stream_id:0 ~stream_seq:0 ~ppid:0x32l ~start_tsn:1000l ~mtu:1200
  );
  ...
  bench "Sctp_reliable.can_send" (fun () ->
    Sctp_reliable.can_send reliable
  );

  bench "Sctp_reliable.alloc_tsn" (fun () ->
    Sctp_reliable.alloc_tsn reliable
  );

  bench "Sctp_reliable.queue_data" (fun () ->
    Sctp_reliable.queue_data reliable chunk
  );

  bench "Sctp_reliable.get_cwnd" (fun () ->
    Sctp_reliable.get_cwnd reliable
  );

  bench "Sctp_reliable.get_flight_size" (fun () ->
    Sctp_reliable.get_flight_size reliable
  );
  ...
  bench "Sctp_full_transport.send_data (1KB)" (fun () ->
    ...
    let t = Sctp_full_transport.create ~host:"127.0.0.1" ~port:(31000 + !send_count mod 1000) () in
