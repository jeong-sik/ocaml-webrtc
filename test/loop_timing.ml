...
let () =
  ...
  let elapsed = Int64.to_float (Int64.sub (time_ns ()) start) /. float_of_int iterations in
  Printf.printf "  Unix.gettimeofday:           %6.0f ns/op\n" elapsed;

  (* 2. get_flight_size + get_cwnd *)
  let start = time_ns () in
  for _ = 1 to iterations do
    ignore (Sctp_full_transport.get_flight_size sender < Sctp_full_transport.get_cwnd sender)
  done;
  let elapsed = Int64.to_float (Int64.sub (time_ns ()) start) /. float_of_int iterations in
  ...
  let start = time_ns () in
  for _ = 1 to min iterations 1000 do  (* Limit to avoid filling buffers *)
    match Sctp_full_transport.send_data sender ~stream_id:0 ~data with
    | Ok _ -> incr send_count
    | Error _ -> ()
  done;
  let elapsed = Int64.to_float (Int64.sub (time_ns ()) start) /. float_of_int !send_count in
  ...
  for _ = 1 to loop_iterations do
    ...
    let can_send = Sctp_full_transport.get_flight_size sender <
                   Sctp_full_transport.get_cwnd sender in
  ...
  Printf.printf "    Check (cwnd):       %6.3f ms (%4.1f%%)\n"
    (!check_time *. 1000.0) (!check_time /. !total_time *. 100.0);
  Printf.printf "    Send:               %6.3f ms (%4.1f%%)\n"
    (!send_time *. 1000.0) (!send_time /. !total_time *. 100.0);
  Printf.printf "    Tick receiver:      %6.3f ms (%4.1f%%)\n"
    (!recv_tick_time *. 1000.0) (!recv_tick_time /. !total_time *. 100.0);
  Printf.printf "    Tick sender:        %6.3f ms (%4.1f%%)\n"
    (!send_tick_time *. 1000.0) (!send_tick_time /. !total_time *. 100.0);
  let overhead = !total_time -. !check_time -. !send_time -. !recv_tick_time -. !send_tick_time in
  Printf.printf "    Loop overhead:      %6.3f ms (%4.1f%%)\n"
    (overhead *. 1000.0) (overhead /. !total_time *. 100.0);
  Printf.printf "\n";
