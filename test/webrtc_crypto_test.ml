...
let () =
  ...
  test "aes_gcm_explicit_nonce_size = 8" (fun () ->
    assert_eq "aes_gcm_explicit_nonce_size" 8 Webrtc.Webrtc_crypto.aes_gcm_explicit_nonce_size
  );
  ...
  test "derive_master_secret produces 48 bytes" (fun () ->
    ...
    let master = Webrtc.Webrtc_crypto.derive_master_secret
  ...
  test "derive_master_secret deterministic" (fun () ->
    ...
    let m1 = Webrtc.Webrtc_crypto.derive_master_secret
      ~pre_master_secret:pre_master
      ~client_random ~server_random
    in
    let m2 = Webrtc.Webrtc_crypto.derive_master_secret
      ~pre_master_secret:pre_master
      ~client_random ~server_random
    in
  ...
  test "derive_key_material produces correct sizes (AES-128-GCM)" (fun () ->
    ...
    let server_random = Cstruct.create 32 in
    let client_random = Cstruct.create 32 in
    let km = Webrtc.Webrtc_crypto.derive_key_material
  ...
  test "encrypt then decrypt returns original" (fun () ->
    ...
    let ciphertext = Webrtc.Webrtc_crypto.aes_gcm_encrypt
      ~key ~implicit_iv ~explicit_nonce ~aad ~plaintext
    in

    (* Ciphertext should be longer (includes 16-byte tag) *)
    assert_true "ciphertext longer"
      (Cstruct.length ciphertext > Cstruct.length plaintext);

    match Webrtc.Webrtc_crypto.aes_gcm_decrypt
      ~key ~implicit_iv ~explicit_nonce ~aad ~ciphertext_and_tag:ciphertext
    with
    | Ok decrypted ->
  ...
  test "decrypt fails with wrong key" (fun () ->
    ...
    match Webrtc.Webrtc_crypto.aes_gcm_decrypt
      ~key:wrong_key ~implicit_iv ~explicit_nonce ~aad ~ciphertext_and_tag:ciphertext
    with
  ...
  test "decrypt fails with tampered ciphertext" (fun () ->
    ...
    match Webrtc.Webrtc_crypto.aes_gcm_decrypt
      ~key ~implicit_iv ~explicit_nonce ~aad ~ciphertext_and_tag:tampered
    with
  ...
  test "decrypt fails with wrong AAD" (fun () ->
    ...
    match Webrtc.Webrtc_crypto.aes_gcm_decrypt
      ~key ~implicit_iv ~explicit_nonce ~aad:wrong_aad ~ciphertext_and_tag:ciphertext
    with
