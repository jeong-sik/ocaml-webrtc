...
let record ~src_port ~dst_port ~initial_tsn ~my_vtag inputs =
  let t = Sctp_core.create ~src_port ~dst_port ~initial_tsn ~my_vtag () in
  let steps =
    List.mapi
      (fun i inp ->
...
let packets_equivalent p1 p2 =
  if has_nondeterministic_content p1 || has_nondeterministic_content p2
  then
    Bytes.length p1 = Bytes.length p2
    && Bytes.length p1 >= 1
    && Bytes.get_uint8 p1 0 = Bytes.get_uint8 p2 0
  else Bytes.equal p1 p2
;;
...
let output_equal a b =
  match a, b with
  | Sctp_core.SendPacket p1, Sctp_core.SendPacket p2 -> packets_equivalent p1 p2
  | DeliverData d1, DeliverData d2 ->
    d1.stream_id = d2.stream_id && Bytes.equal d1.data d2.data
  | SetTimer s1, SetTimer s2 -> s1.timer = s2.timer && Float.equal s1.delay_ms s2.delay_ms
...
let outputs_equal a b = List.length a = List.length b && List.for_all2 output_equal a b
...
let verify trace =
  ...
  let rec check idx original replayed =
    ...
    | orig :: rest_o, repl :: rest_r ->
      if outputs_equal orig.outputs repl.outputs
      then check (idx + 1) rest_o rest_r
      else Error (idx, orig.outputs, repl.outputs)
    | _ -> Error (-1, [], [])
    (* length mismatch *)
  in
...
let pp_divergence idx expected actual =
  Printf.sprintf
    "Step %d diverged: expected %d outputs, got %d"
    idx
