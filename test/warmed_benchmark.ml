...
let () =
  ...
  Printf.printf "  cwnd:     %d bytes (%d packets)\n"
  ...
  while Unix.gettimeofday () < warmup_end do
    (* Try to send *)
    if Sctp_full_transport.get_flight_size sender <
       Sctp_full_transport.get_cwnd sender then begin
  ...
  Printf.printf "  cwnd after warmup:     %d bytes (%d packets)\n"
    (Sctp_full_transport.get_cwnd sender)
    (Sctp_full_transport.get_cwnd sender / packet_size);
  Printf.printf "  ssthresh after warmup: %d bytes\n" (Sctp_full_transport.get_ssthresh sender);
  Printf.printf "\n";
  ...
  Printf.printf "║  - OCaml SCTP Warmed:  %.2f MB/s                           ║\n" throughput_mbps;
