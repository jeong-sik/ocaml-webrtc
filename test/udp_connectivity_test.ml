...
let () =
  ...
  Printf.printf "  sender: sent=%d recv=%d errors=%d\n"
  ...
  Printf.printf "  sender packets_sent: %d\n%!"
    (Udp_transport.get_stats sender_udp).packets_sent;

  (* Receive on receiver side *)
  let buf = Bytes.make 200 '\000' in
  Unix.sleepf 0.001;  (* Brief pause *)
  ...
  Printf.printf "  receiver packets_recv: %d\n\n%!"
    (Udp_transport.get_stats receiver_udp).packets_recv;
  ...
  Printf.printf "  sender: sent=%d recv=%d errors=%d\n"
    s1.packets_sent s1.packets_recv s1.send_errors;
  Printf.printf "  receiver: sent=%d recv=%d\n"
  ...
  Printf.printf "  packets_sent: %d -> %d (delta: %d)\n%!"
  ...
  Printf.printf "  packets_recv: %d -> %d (delta: %d)\n\n%!"
  ...
  let s_after = (Udp_transport.get_stats sender_udp).packets_sent in
  Printf.printf "  UDP packets_sent: %d -> %d (delta: %d)\n%!"
    s_before s_after (s_after - s_before);
