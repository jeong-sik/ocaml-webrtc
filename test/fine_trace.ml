...
let () =
  ...
  for step = 1 to 40000 do
    ...
    if delta > !prev_delta then begin
      ...
      Printf.printf "step %d: delta %d->%d sent=%d recv=%d cwnd=%d flight=%d rtx=%d\n%!"
        step !prev_delta delta s_udp.packets_sent r_udp.packets_recv cwnd flight s.retransmissions;
      prev_delta := delta;
    ...
    if step mod 10000 = 0 then begin
      Printf.printf "[checkpoint step %d: msgs=%d rtx=%d]\n%!" step !msgs s.retransmissions
    end
  done;
