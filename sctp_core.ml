...
let next_stream_seq t stream_id =
  let current =
    match Hashtbl.find_opt t.stream_seq_out stream_id with
    | Some v -> v
    | None -> 0
  in
...
let reset_stream_seq_out t stream_id = Hashtbl.replace t.stream_seq_out stream_id 0

let reset_stream_seq_in t stream_id = Hashtbl.replace t.stream_seq_in stream_id 0
...
let schedule_pending_reset t target request_seq last_tsn streams =
  Hashtbl.replace
    ...
    request_seq
    { request_seq; last_tsn; streams; target }
;;
  ...
  List.iter
    (function
      | Sctp_reconfig.Outgoing_ssn_reset { request_seq; streams; last_tsn; _ } ->
        if reconfig_ready t last_tsn
        then (
          apply_stream_reset t Reset_incoming streams;
          responses
          := build_reconfig_response request_seq Sctp_reconfig.result_success
             :: !responses)
        else (
          schedule_pending_reset t Reset_incoming request_seq last_tsn streams;
          responses
          := build_reconfig_response request_seq Sctp_reconfig.result_in_progress
             :: !responses)
      ...
      | Sctp_reconfig.Add_outgoing_streams { request_seq; _ } ->
        responses
        := build_reconfig_response request_seq Sctp_reconfig.result_success :: !responses
      | Sctp_reconfig.Add_incoming_streams { request_seq; _ } ->
        responses
        := build_reconfig_response request_seq Sctp_reconfig.result_success :: !responses
      | Sctp_reconfig.Reconfig_response { response_seq; result } ->
        (match Hashtbl.find_opt t.pending_outgoing_resets response_seq with
         | None -> ()
         | Some streams ->
           if result = Sctp_reconfig.result_success
           then (
             apply_stream_reset t Reset_outgoing streams;
             Hashtbl.remove t.pending_outgoing_resets response_seq)
           else if result <> Sctp_reconfig.result_in_progress
           then Hashtbl.remove t.pending_outgoing_resets response_seq)
...
let flush_pending_reconfigs t =
  ...
  Hashtbl.iter
    (fun _ pending ->
       if reconfig_ready t pending.last_tsn then ready := pending :: !ready)
    t.pending_incoming_resets;
  ...
  | pendings ->
    ...
    List.iter
      (fun pending ->
         ...
         responses
         := build_reconfig_response pending.request_seq Sctp_reconfig.result_success
            :: !responses)
      pendings;
...
let process_data_chunk t chunk =
  ...
  then (
    ...
    outputs
    := DeliverData { stream_id = chunk.Sctp.stream_id; data = chunk.Sctp.user_data }
       :: !outputs);
  (* Delayed ACK - RFC 4960 §6.2 *)
  t.packets_since_sack <- t.packets_since_sack + 1;
  if t.packets_since_sack >= 2
  then (
...
let process_sack_chunk t sack =
  ...
  if Sctp_reliable.all_acked t.reliable then outputs := CancelTimer T3Rtx :: !outputs;
  (* RFC 4960 §9.2: If ShutdownPending and all data acked, send SHUTDOWN *)
  if t.conn_state = ShutdownPending && Sctp_reliable.get_flight_size t.reliable = 0
  then (
    t.conn_state <- ShutdownSent;
    let cumulative_tsn = Sctp_reliable.get_cumulative_tsn t.reliable in
    let shutdown_chunk =
      Sctp_shutdown.encode_shutdown { cumulative_tsn_ack = cumulative_tsn }
    in
...
let process_single_chunk t packet offset =
  ...
  then [], packet_len (* Not enough for chunk header *)
  else (
    ...
    let chunk_data =
      ...
      then Bytes.sub packet offset chunk_len
      else Bytes.sub packet offset (packet_len - offset)
    in
    let outputs =
      match chunk_type with
      | 0 ->
        (* DATA chunk *)
        (match Sctp.decode_data_chunk chunk_data with
         | Ok chunk -> process_data_chunk t chunk
         | Error e -> [ Error ("DATA decode: " ^ e) ])
      | 3 ->
        (* SACK chunk *)
        (match Sctp_reliable.decode_sack chunk_data with
         | Ok sack -> process_sack_chunk t sack
         | Error e -> [ Error ("SACK decode: " ^ e) ])
      | 1 ->
        (* INIT - we're server - RFC 4960 §5.1 *)
        (match Sctp_handshake.server_process_init chunk_data with
         | Ok (_local_params, init_ack) ->
           (* Server remains STATELESS - cookie holds all state *)
           let bundle =
             { Sctp_bundling.chunks = [ init_ack ]; total_size = Bytes.length init_ack }
           in
           let packet =
      ...
      | 2 ->
        (* INIT-ACK - we're client - RFC 4960 §5.1 *)
        (match t.handshake_params with
         | None -> [ Error "Received INIT-ACK but no handshake in progress" ]
         | Some local_params ->
           (match Sctp_handshake.client_process_init_ack chunk_data local_params with
            | Ok (assoc, cookie_echo) ->
              ...
              let bundle =
                { Sctp_bundling.chunks = [ cookie_echo ]
                ; total_size = Bytes.length cookie_echo
                }
              in
              let packet =
                Sctp_bundling.assemble_packet
      ...
      | 10 ->
        (* COOKIE-ECHO - we're server - RFC 4960 §5.1 *)
        (match Sctp_handshake.server_process_cookie_echo chunk_data with
         | Ok (assoc, cookie_ack) ->
           ...
           let bundle =
             { Sctp_bundling.chunks = [ cookie_ack ]
             ; total_size = Bytes.length cookie_ack
             }
           in
           let packet =
             Sctp_bundling.assemble_packet
      ...
      | 11 ->
        (* COOKIE-ACK - we're client - RFC 4960 §5.1 *)
        if t.conn_state = CookieEchoed
        then (
          t.conn_state <- Established;
          [ ConnectionEstablished ])
        else [ Error "Received COOKIE-ACK in unexpected state" ]
      | 4 ->
        (* HEARTBEAT *)
        (match Sctp_heartbeat.process_heartbeat chunk_data with
      ...
      | 6 ->
        ...
        let error_msg =
          if Bytes.length chunk_data > 4
          then
            Printf.sprintf
              "Association aborted by peer (T-bit=%b, %d error cause bytes)"
              t_bit
              (Bytes.length chunk_data - 4)
          else Printf.sprintf "Association aborted by peer (T-bit=%b)" t_bit
        in
        [ Error error_msg; ConnectionClosed ]
      | 9 ->
        (* ERROR - RFC 4960 §3.3.10 *)
      ...
      | 7 ->
        ...
        (match Sctp_shutdown.decode_shutdown chunk_data with
         | Ok _shutdown ->
         ...
         | Error _ -> [])
      | 8 ->
        (* SHUTDOWN-ACK - RFC 4960 §9.2 *)
        (match Sctp_shutdown.decode_shutdown_ack chunk_data with
         | Ok () ->
         ...
         | Error _ -> [])
      | 14 ->
        (* SHUTDOWN-COMPLETE - RFC 4960 §9.2 *)
        (match Sctp_shutdown.decode_shutdown_complete chunk_data with
         | Ok _t_bit ->
           t.conn_state <- Closed;
      ...
      | chunk_type_id when chunk_type_id = Sctp.int_of_chunk_type Sctp.RE_CONFIG ->
        ...
        let raw =
          { Sctp.chunk_type; chunk_flags; chunk_length = chunk_len; chunk_value }
        in
      ...
      | _ ->
        ...
        (match action with
         | Sctp_error.StopDiscard ->
           ...
           [ Error
               (Printf.sprintf "Unknown chunk type %d (action=stop-discard)" chunk_type)
           ]
         | Sctp_error.StopDiscardReport ->
           (* 01: Stop processing, send ERROR with unrecognized chunk *)
           let error_chunk =
             Sctp_error.make_unrecognized_chunk_error ~unrecognized_chunk:chunk_data
           in
           [ SendPacket error_chunk
           ; Error
               (Printf.sprintf "Unknown chunk type %d (action=stop-report)" chunk_type)
           ]
...
let process_packet t packet =
  ...
  then [ Error "Packet too short for SCTP header" ]
    ...
    if first_byte <= 14 && packet_len < 100
    then (
      ...
      then
        [ Error
            (Printf.sprintf
               "Vtag mismatch: got %ld, peer=%ld, my=%ld"
               vtag
               t.peer_vtag
               t.my_vtag)
        ]
        ...
        let verify_checksum =
          if t.config.skip_checksum_validation
          then (
            (* TESTING ONLY: Skip checksum validation - warn once *)
            if not !checksum_bypass_warned
            then (
              Log.warn "[SECURITY WARNING] Checksum validation disabled - TESTING ONLY!";
            ...
            Bytes.set_int32_be packet 8 received_checksum;
            (* Restore *)
            calculated = received_checksum)
        in
        if not verify_checksum
        then [ Error (Printf.sprintf "CRC32c mismatch") ]
        else (
...
let queue_user_data t ~stream_id ~data =
  if not (Sctp_reliable.can_send t.reliable)
  then [ Error "Congestion window full" ]
  else (
    let outputs = ref [] in
   ...
   | Some sack_chunk ->
     ...
     let packet =
       Sctp_bundling.assemble_packet
         ...
         ~src_port:t.src_port
         ~dst_port:t.dst_port
         { Sctp_bundling.chunks = [ sack_chunk ]
         ; total_size = Bytes.length sack_chunk + 12
         }
     in
     outputs := SendPacket packet :: !outputs;
     t.pending_sack_chunk <- None
...
let has_pending_transmit t =
  t.pending_sack_chunk <> None || Sctp_bundling.pending_count t.bundler > 0
;;
...
let handle t input =
  match input with
  | PacketReceived packet -> process_packet t packet
  | TimerFired timer ->
    (match timer with
  ...
  | UserSend { stream_id; data } -> queue_user_data t ~stream_id ~data
  | UserResetStreams { stream_ids } ->
    if t.conn_state <> Established
    then [ Error "RE-CONFIG requires Established state" ]
    else (
      ...
      let param =
        Sctp_reconfig.Outgoing_ssn_reset
          { request_seq; response_seq = 0l; last_tsn; streams = stream_ids }
      in
      let packet = build_reconfig_packet t [ param ] in
      [ SendPacket packet ])
  | UserClose ->
    (* RFC 4960 §9.2: Graceful Shutdown *)
    if Sctp_reliable.get_flight_size t.reliable = 0
    then (
      ...
      in
      let rto = Sctp_reliable.get_rto t.reliable in
      [ SendPacket shutdown_chunk
      ; SetTimer { timer = Shutdown; delay_ms = rto *. 1000.0 }
      ])
    else (
...
let initiate t =
  ...
  let bundle =
    { Sctp_bundling.chunks = [ init_chunk ]; total_size = Bytes.length init_chunk }
  in
  let packet =
    Sctp_bundling.assemble_packet
...
let pp_output fmt = function
  ...
  | CancelTimer timer -> Format.fprintf fmt "CancelTimer(%s)" (string_of_timer timer)
...
let pp_mutable_stats fmt (s : mutable_stats) =
  Format.fprintf
    fmt
    "msgs=%d/%d bytes=%d/%d sacks=%d/%d rtx=%d fast=%d"
    s.ms_messages_sent
    s.ms_messages_recv
    s.ms_bytes_sent
    s.ms_bytes_recv
    s.ms_sacks_sent
    s.ms_sacks_recv
    s.ms_retransmissions
    s.ms_fast_retransmissions
;;
