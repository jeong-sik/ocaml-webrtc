...
let derive_key ~master ~label ~key_derivation_rate ~index ~out_len =
  if out_len < 0
  then Error "Invalid output length"
  else if out_len = 0
  then Ok Bytes.empty
  else if index < 0L || index > 0xFFFFFFFFFFFFL
  then Error "Index must be 48-bit"
  else if
    Bytes.length master.key <> 16
    && Bytes.length master.key <> 24
    && Bytes.length master.key <> 32
  then Error "Master key length must be 16/24/32 bytes"
  else if Bytes.length master.salt <> 14
  then Error "Master salt length must be 14 bytes"
  else (
    let r =
      if key_derivation_rate = 0L then 0L else Int64.div index key_derivation_rate
    in
...
let derive_session_keys ~profile ~master ~key_derivation_rate ~index =
  ...
  in
  match
    ( derive 0x00 params.cipher_key_len
    ...
    , derive 0x05 params.cipher_salt_len )
  with
  | Ok srtp_key, Ok srtp_auth, Ok srtp_salt, Ok srtcp_key, Ok srtcp_auth, Ok srtcp_salt ->
    Ok
...
let srtp_iv ~salt ~ssrc ~index =
  if Bytes.length salt <> 14
  then Error "SRTP salt must be 14 bytes"
  else if index < 0L || index > 0xFFFFFFFFFFFFL
  then Error "SRTP index must be 48-bit"
  else (
    let iv = Bytes.create 16 in
...
let aes_cm_crypt ~key ~iv ~payload =
  if Bytes.length iv <> 16
  then Error "IV must be 16 bytes"
  else if Bytes.length key <> 16 && Bytes.length key <> 24 && Bytes.length key <> 32
  then Error "AES-CM key must be 16/24/32 bytes"
  ...
  then Ok Bytes.empty
    ...
    let ciphertext =
      Mirage_crypto.AES.CTR.encrypt ~key:aes_key ~ctr (Bytes.to_string payload)
    in
...
let hmac_sha1 ~key ~data =
  ...
  let raw =
    H.hmac_string ~key:(Bytes.to_string key) (Bytes.to_string data) |> H.to_raw_string
  in
  Bytes.of_string raw
;;
...
let srtp_index ~roc ~seq =
  if seq < 0 || seq > 0xFFFF
  then Error "Sequence must be 16-bit"
  else (
...
let srtp_gcm_iv ~salt ~ssrc ~index =
  if Bytes.length salt <> 12
  then Error "GCM salt must be 12 bytes"
  else if index < 0L || index > 0xFFFFFFFFFFFFL
  then Error "SRTP index must be 48-bit"
  else (
...
let aes_gcm_encrypt ~key ~iv ~aad ~plaintext =
  if Bytes.length iv <> 12
  then Error "AES-GCM IV must be 12 bytes"
  else if Bytes.length key <> 16 && Bytes.length key <> 32
  then Error "AES-GCM key must be 16 or 32 bytes"
  else (
...
let aes_gcm_decrypt ~key ~iv ~aad ~ciphertext_and_tag =
  ...
  then Error "AES-GCM key must be 16 or 32 bytes"
  else if Bytes.length ciphertext_and_tag < gcm_tag_len
  then Error "Ciphertext too short for GCM tag"
  else (
    let key_str = Bytes.to_string key in
    let iv_str = Bytes.to_string iv in
    let aad_str = Bytes.to_string aad in
     ...
     | Ok index ->
       (match srtp_gcm_iv ~salt:keys.srtp_salt_key ~ssrc:header.ssrc ~index with
        ...
        | Ok iv ->
          ...
          (match
             aes_gcm_encrypt ~key:keys.srtp_encryption_key ~iv ~aad ~plaintext:payload
           with
           | Error e -> Error e
...
let unprotect_rtp_gcm ~keys ~roc ~packet =
  if Bytes.length packet <= 12 + gcm_tag_len
  then Error "SRTP-GCM packet too short"
  else (
       ...
       | Ok index ->
         (match srtp_gcm_iv ~salt:keys.srtp_salt_key ~ssrc:header.Rtp.ssrc ~index with
          ...
          | Ok iv ->
            let aad = Bytes.sub packet 0 header_len in
            let ciphertext_and_tag =
              Bytes.sub packet header_len (Bytes.length packet - header_len)
            in
            (match
               aes_gcm_decrypt ~key:keys.srtp_encryption_key ~iv ~aad ~ciphertext_and_tag
             with
             | Error e -> Error e
             | Ok plaintext ->
...
let protect_rtcp_gcm ~keys ~index ~packet =
  if Bytes.length packet < 8
  then Error "RTCP packet too short"
  else (
    let idx32 = Int32.logand index 0x7FFFFFFFl in
    ...
    | Ok iv ->
      ...
      let e_index = Int32.logor idx32 0x80000000l in
      (* E=1 for encrypted *)
      ...
      (match
         aes_gcm_encrypt ~key:keys.srtcp_encryption_key ~iv ~aad ~plaintext:payload
       with
       | Error e -> Error e
       | Ok ciphertext_and_tag ->
...
let unprotect_rtcp_gcm ~keys ~packet =
  ...
  if Bytes.length packet < 28
  then Error "SRTCP-GCM packet too short"
  else (
    ...
    | Ok iv ->
      ...
      (match
         aes_gcm_decrypt ~key:keys.srtcp_encryption_key ~iv ~aad ~ciphertext_and_tag
       with
       | Error e -> Error e
       | Ok plaintext ->
...
let protect_rtp ~profile ~keys ~roc ~packet =
  ...
  if is_aead_profile profile
  then protect_rtp_gcm ~keys ~roc ~packet
  else (
    let params = params_of_profile profile in
    ...
    | Ok { Rtp.header; payload } ->
      ...
      (match index_result with
       ...
       | Ok index ->
         let payload_result =
           if params.cipher_key_len = 0
           then Ok payload
           else (
             match srtp_iv ~salt:keys.srtp_salt_key ~ssrc:header.ssrc ~index with
             | Error e -> Error e
             | Ok iv -> aes_cm_crypt ~key:keys.srtp_encryption_key ~iv ~payload)
         in
...
let unprotect_rtp ~profile ~keys ~roc ~packet =
  (* AEAD profiles use GCM-specific path *)
  if is_aead_profile profile
  then unprotect_rtp_gcm ~keys ~roc ~packet
  else (
    let params = params_of_profile profile in
    let tag_len = params.srtp_auth_tag_len in
    if Bytes.length packet <= tag_len
    then Error "SRTP packet too short"
    else (
      ...
      let expected =
        srtp_auth_tag ~auth_key:keys.srtp_auth_key ~packet:rtp_packet ~roc ~tag_len
      in
      if not (constant_time_eq tag expected)
      then Error "SRTP authentication failed"
      else (
...
let protect_rtcp ~profile ~keys ~index ~encrypt ~packet =
  ...
  if is_aead_profile profile
  then protect_rtcp_gcm ~keys ~index ~packet
  else (
    let params = params_of_profile profile in
    let tag_len = params.srtcp_auth_tag_len in
    let idx32 = Int32.logand index 0x7FFFFFFFl in
    if Int32.compare index 0l < 0 || Int32.compare index 0x7FFFFFFFl > 0
    then Error "SRTCP index must be 31-bit"
    else if Bytes.length packet < 8
    then Error "RTCP packet too short"
    else (
      let ssrc = read_uint32_be packet 4 in
      let encrypted_result =
        if (not encrypt) || params.cipher_key_len = 0
        then Ok packet
        else (
      ...
      | Ok encrypted_packet ->
        ...
        let tag =
          srtcp_auth_tag ~auth_key:keys.srtcp_auth_key ~packet:auth_input ~tag_len
        in
...
let unprotect_rtcp ~profile ~keys ~packet =
  (* AEAD profiles use GCM-specific path *)
  if is_aead_profile profile
  then unprotect_rtcp_gcm ~keys ~packet
  else (
    let params = params_of_profile profile in
    let tag_len = params.srtcp_auth_tag_len in
    if Bytes.length packet < 12 + tag_len
    then Error "SRTCP packet too short"
    else (
      ...
      in
      if not (constant_time_eq tag expected)
      then Error "SRTCP authentication failed"
      else (
        ...
        then Error "RTCP packet too short"
        else if (not encrypt) || params.cipher_key_len = 0
        then Ok (base, idx32)
        else (
