...
let sctp_common_header_size =
  12 (* Source port(2) + Dest port(2) + Vtag(4) + Checksum(4) *)
;;
...
let add_chunk t chunk =
  ...
  then (
    ...
    let flushed = { chunks = List.rev t.pending_chunks; total_size = t.pending_size } in
...
let flush t =
  ...
  then None
    ...
    let bundle = { chunks = List.rev t.pending_chunks; total_size = t.pending_size } in
    t.pending_chunks <- [];
...
let assemble_packet ~vtag ~src_port ~dst_port bundle =
  let total_chunk_size =
    List.fold_left
      (fun acc chunk -> acc + padded_size (Bytes.length chunk))
      0
      bundle.chunks
  in
...
let pp fmt t =
  Format.fprintf
    fmt
    "Bundler{mtu=%d, pending=%d chunks (%d bytes), available=%d}"
    t.mtu
